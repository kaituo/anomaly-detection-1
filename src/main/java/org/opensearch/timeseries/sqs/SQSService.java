...
public abstract class SQSService {
    ...
    public SQSService(Settings settings, String queueUrl) {
        this(settings, queueUrl, null);
    }

    @org.opensearch.timeseries.annotation.SuppressForbidden(reason = "java.security.AccessController usage: required for privileged AWS client initialization.")
    SQSService(Settings settings, String queueUrl, SqsClient sqsClient) {
        ...
        if (sqsClient == null) {
            // use default credentials provider to receive messages as sqs queue's allow policy
            // trusts our service principal.
            this.sqsClient = AccessController.doPrivileged((PrivilegedAction<SqsClient>) () -> {
                AwsCredentialsProvider credentialsProvider = SecurityUtil.createCredentialsProvider();
                return SqsClient.builder().region(Region.of(region)).credentialsProvider(credentialsProvider).build();
            });
        } else {
            // Package-private constructor path used by tests to inject a mock SQS client.
     ...
     *
     * @return Redrive policy metadata or null if not configured
     * @throws IllegalStateException if the queue attributes cannot be fetched or parsed
     */
    public RedrivePolicy getRedrivePolicy() {
        try {
            ...
            throw new IllegalStateException(
                "Failed to fetch redrive policy for SQS queue "
                    + queueUrl
                    + " with sqs:GetQueueAttributes; cannot verify DLQ configuration"
                    + formatAwsError(e),
                e
            );
        } catch (Exception e) {
            logger.warn("Failed to fetch or parse redrive policy for SQS queue {}", queueUrl, e);
            throw new IllegalStateException(
                "Failed to fetch or parse redrive policy for SQS queue " + queueUrl + "; cannot verify DLQ configuration" + formatCause(e),
                e
            );
        }
    }

    private static String formatAwsError(SqsException e) {
        if (e.awsErrorDetails() == null) {
            return formatCause(e);
        }
    ...
    private static String formatCause(Exception e) {
