...
public abstract class EventBridgeHandler {
    ...
    private static final String HOURLY_CRON_SCHEDULE_GROUP = "timeseries";
    ...
    private static final String DAILY_S3_CLEANUP_SCHEDULE_GROUP = "timeseries";
    ...
    private CompletableFuture<Void> createOrUpdateHourlyCronAsync() {
        ...
        CreateScheduleRequest createScheduleRequest = CreateScheduleRequest
            .builder()
            .groupName(HOURLY_CRON_SCHEDULE_GROUP)
        ...
        schedulerClient
            .getSchedule(GetScheduleRequest.builder().groupName(HOURLY_CRON_SCHEDULE_GROUP).name(HOURLY_CRON_SCHEDULE_NAME).build())
            ...
            if (updateError == null) {
                ...
                if (updateCause instanceof ConflictException) {
                    schedulerClient
                        ...
                        .whenComplete((existingSchedule, getError) -> {
                            if (getError == null && existingSchedule != null
                                && ScheduleState.ENABLED.equals(existingSchedule.state())) {
                                logger.info("Hourly cron schedule already enabled; treating as success.");
                                result.complete(null);
                            } else {
                ...
                if (existingScheduleExpression == null || existingFlexibleWindow == null || existingTarget == null) {
                    result
                        .completeExceptionally(
                            new IllegalStateException("Existing hourly cron schedule is missing required fields")
                        );
                    return;
                }
    ...
    private CompletableFuture<Void> createOrUpdateDailyS3CleanupAsync() {
        ...
        CreateScheduleRequest createScheduleRequest = CreateScheduleRequest
            .builder()
            .groupName(DAILY_S3_CLEANUP_SCHEDULE_GROUP)
        ...
        CompletableFuture<Void> result = new CompletableFuture<>();
        schedulerClient
            .getSchedule(GetScheduleRequest.builder().groupName(DAILY_S3_CLEANUP_SCHEDULE_GROUP).name(DAILY_S3_CLEANUP_SCHEDULE_NAME).build())
            .whenComplete((existingSchedule, getError) -> {
                if (getError == null) {
                    if (existingSchedule != null && ScheduleState.ENABLED.equals(existingSchedule.state())) {
        ...
        schedulerClient
            ...
            .whenComplete((existingSchedule, throwable) -> {
                ...
                if (existingScheduleExpression == null || existingFlexibleWindow == null || existingTarget == null) {
                    result
                        .completeExceptionally(
                            new IllegalStateException("Existing daily S3 cleanup schedule is missing required fields")
                        );
                    return;
                }
