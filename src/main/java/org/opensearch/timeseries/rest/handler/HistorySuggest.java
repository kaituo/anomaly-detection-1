...
public class HistorySuggest {
    ...
    public HistorySuggest(Config config,
            User user, SearchFeatureDao searchFeatureDao,
    ...
    public void suggestHistory(ActionListener<SuggestConfigParamResponse> listener) {
        if (intervalRecommended == null) {
            listener.onResponse(new SuggestConfigParamResponse.Builder()
                    .history(config.getDefaultHistory()).build());
            return;
        }
    ...
    private int getNumberOfIntervals(
        ...
        long endEpochMillis,
        IntervalTimeConfiguration intervalConfig
    ) {
