...
public abstract class SDKDataManagement<IndexType extends Enum<IndexType> & TimeSeriesIndex, IndexableResultType extends IndexableResult>
    ...
    public void doesResultIndexExists(String indexName, ActionListener<Boolean> listener, String tenantId) {
        if (isAossCustomResultAliasName(indexName)) {
            existenceRequest("/_alias/" + indexName, tenantId, listener);
            return;
        }
        existenceRequest("/" + indexName, tenantId, listener);
    }

    @Override
    public void doesResultAliasExists(String aliasName, ActionListener<Boolean> listener, String tenantId) {
        existenceRequest("/_alias/" + aliasName, tenantId, listener);
    }
    ...
    /**
     * Get the rest client for the given tenant id from the shared cache in
     * {@link RestClientProvider}.
     * Reuses clients per endpoint so we don’t re-resolve and re-handshake (TCP/TLS)
     * on every call.
     *
     * @param tenantId tenant id
     * @return the rest client for the given tenant id
     * @throws RuntimeException if failed to create the rest client for the given
     *                          tenant id
     */
    ...
    private void existenceRequest(String path, String tenantId, ActionListener<Boolean> listener) {
        try {
    ...
    private <T> void retryDeleteDummyResultOnConcreteIndex(
        ...
        deleteDummyResult(restClient, fallbackConcreteIndex, requestDummyId, null, writeResponseDetails, function, listener);
    }
    ...
    public void initCustomResultIndexDirectly(
        ...
        try {
            ...
            String createIndexName = aossDataPlane
                ? getConcreteCustomResultIndexName(resultIndexAlias)
            ...
            String bodyEntity = "{\"mappings\":" + resultMapping + ",\"aliases\":{\"" + resultIndexAlias + "\":{}}}";
            Response response = sendWithBody("PUT", "/" + createIndexName, bodyEntity, tenantId);
            ...
            if (!aossDataPlane && isInvalidIndexName(response, responseBody)) {
                ...
                response = sendWithBody("PUT", "/" + fallbackIndexName, bodyEntity, tenantId);
        ...
        doesResultIndexOrAliasExists(resultIndexOrAlias, ActionListener.wrap(exists -> {
            ...
            initCustomResultIndexDirectly(resultIndexOrAlias, ActionListener.wrap(response -> {
                if (response.isAcknowledged()) {
                    ...
                    // already exists.
                    validateExistingResultIndexAfterCreateAttempt(resultIndexOrAlias, function, listener, tenantId, null);
                }
            }, exception -> {
                if (ExceptionsHelper.unwrapCause(exception) instanceof ResourceAlreadyExistsException) {
                    validateResultIndexAndExecute(resultIndexOrAlias, function, false, listener, tenantId);
                } else {
                    validateExistingResultIndexAfterCreateAttempt(resultIndexOrAlias, function, listener, tenantId, exception);
                }
            }), tenantId);
        }, listener::onFailure), tenantId);
    }
