...
public abstract class IndexJobActionHandler<IndexType extends Enum<IndexType> & TimeSeriesIndex, IndexManagementType extends IndexManagement<IndexType>, TaskCacheManagerType extends TaskCacheManager, TaskTypeEnum extends TaskType, TaskClass extends TimeSeriesTask, TaskManagerType extends TaskManager<TaskCacheManagerType, TaskTypeEnum, TaskClass, IndexType, IndexManagementType>, IndexableResultType extends IndexableResult, ExecuteResultResponseRecorderType extends ExecuteResultResponseRecorder<IndexType, IndexManagementType, TaskCacheManagerType, TaskTypeEnum, TaskClass, TaskManagerType, IndexableResultType>> {
            ...
            try (XContentParser parser = createXContentParserFromRegistry(xContentRegistry, response.getSourceAsBytesRef())) {
                ...
                if (currentAdJob.isEnabled()) {
                    ...
                    startConfig(
                        ...
                        ActionListener
                            .wrap(
                                r -> { indexJob(newJob, null, listener); },
                                e -> {
    ...
    public void startConfig(
        ...
        try {
            if (indexManagement.doesStateIndexExist()) {
                ...
                taskManager.getAndExecuteOnLatestConfigLevelTask(config, detectionDateRange, user, transportService, listener);
            } else {
                ...
                indexManagement.initStateIndex(ActionListener.wrap(r -> {
                    if (r.isAcknowledged()) {
                        ...
                        taskManager.updateLatestFlagOfOldTasksAndCreateNewTask(config, detectionDateRange, user, listener);
                    } else {
    ...
    public void stopJob(String configId, TransportService transportService, ActionListener<JobResponse> listener) {
        ...
        client.get(getRequest, ActionListener.wrap(response -> {
            if (response.isExists()) {
                ...
                listener.onFailure(new OpenSearchStatusException("job not exist: " + configId, RestStatus.BAD_REQUEST));
            }
        }, exception -> listener.onFailure(exception)));
    }
        ...
        return new ActionListener<StopConfigResponse>() {
            ...
            public void onResponse(StopConfigResponse stopDetectorResponse) {
                if (stopDetectorResponse.success()) {
                    ...
                    logger.error("Failed to delete model for detector {}", configId);
    ...
    /**
     * Start detector. Will create schedule job for realtime analysis,
     * and start task for historical detector.
     *
            ...
            if (historical) {
                ...
                taskManager
                    .getAndExecuteOnLatestConfigLevelTask(
                        configId,
                        getHistorialConfigTaskTypes(),
                        (task) -> taskManager.stopHistoricalAnalysis(configId, task, user, listener),
                        transportService,
                        false,// don't need to reset task state when stop config
                        listener
                    );
            } else {
    ...
    protected abstract List<TaskTypeEnum> getHistorialConfigTaskTypes();
}
