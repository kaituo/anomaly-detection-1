...
//                    .orElse(null);
//                // choose median as top 1 entity may be too dense. We won't have a representative recommendation for other entities.
//                topKeys.put(config.getCategoryFields().get(0), key);

                List<? extends Terms.Bucket> sortedBuckets = entities.getBuckets()
                        .stream()
...
//                    );
                ...
                List<? extends CompositeAggregation.Bucket> sortedCompositeBuckets = compositeAgg.getBuckets()
                    .stream()
