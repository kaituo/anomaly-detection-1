...
public interface CheckpointDaoInterface<RCFModelType> {
     ...
     */
    default ModelState<RCFModelType> processHCGetResponse(GetResponse response, String modelId, String configId) {
        Optional<Map<String, Object>> checkpointString = processRawCheckpoint(response);
        if (checkpointString.isPresent()) {
            return getCodec().fromEntityModelCheckpoint(checkpointString.get(), modelId, configId);
        } else {
            return null;
        }
    }
