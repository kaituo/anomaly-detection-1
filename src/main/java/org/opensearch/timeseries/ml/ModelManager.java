...
public abstract class ModelManager<RCFModelType extends ThresholdedRandomCutForest, IndexableResultType extends IndexableResult, IntermediateResultType extends IntermediateResult<IndexableResultType>, IndexType extends Enum<IndexType> & TimeSeriesIndex, DataManagementType extends DelegatingDataManagement<IndexType>, CheckpointDaoType extends CheckpointDaoInterface<RCFModelType>, ColdStarterType extends ModelColdStart<RCFModelType, IndexType, DataManagementType, IndexableResultType>> {
    ...
    private boolean isModelForConfig(String detectorId, String tenantId, String modelId, Object modelState) {
        if (modelState instanceof ModelState<?> state && state.getConfigId() != null) {
            ...
            return tenantMatches(tenantId, state.getTenantId(), modelId);
        }

        return modelIdMatchesConfig(detectorId, tenantId, modelId);
    }

    private boolean tenantMatches(String expectedTenantId, String modelTenantId, String modelId) {
        if (expectedTenantId == null || expectedTenantId.isEmpty()) {
            return true;
        }
        if (modelTenantId != null && modelTenantId.isEmpty() == false) {
            return expectedTenantId.equals(modelTenantId);
        }
        return modelIdMatchesTenant(expectedTenantId, modelId);
    }

    private boolean modelIdMatchesConfig(String detectorId, String tenantId, String modelId) {
        try {
            return detectorId.equals(SingleStreamModelIdMapper.getConfigIdForModelId(modelId));
        } catch (IllegalArgumentException e) {
            try {
                Triple<String, String, String> entityModelParts = Entity.extractTenantIdAndConfigId(modelId);
                if (detectorId.equals(entityModelParts.getMiddle()) == false) {
                    return false;
                }
                return tenantId == null || tenantId.isEmpty() || StringUtil.sanitizeId(tenantId).equals(entityModelParts.getLeft());
            } catch (IllegalArgumentException entityModelException) {
                LOG.debug("Skipping unrecognized model id [{}] while clearing [{}]", modelId, detectorId, entityModelException);
                return false;
            }
        }
    }

    private boolean modelIdMatchesTenant(String tenantId, String modelId) {
        try {
            Triple<String, String, String> entityModelParts = Entity.extractTenantIdAndConfigId(modelId);
            return StringUtil.sanitizeId(tenantId).equals(entityModelParts.getLeft());
        } catch (IllegalArgumentException e) {
            return true;
        }
    }
