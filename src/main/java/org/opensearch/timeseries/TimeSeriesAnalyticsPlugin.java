...
public class TimeSeriesAnalyticsPlugin extends Plugin implements ActionPlugin, ScriptPlugin, JobSchedulerExtension {
    ...
    private SecurityClientUtil adSecurityClientUtil;
    private SecurityClientUtil forecastSecurityClientUtil;
    ...
    private ADNodeStateManager adStateManager;
    private ForecastNodeStateManager forecastStateManager;
    ...
    public List<RestHandler> getRestHandlers(
        ...
        jobRunner.setNodeStateManager(adStateManager);
        ...
        RestStatsAnomalyDetectorAction statsAnomalyDetectorAction = new RestStatsAnomalyDetectorAction(timeSeriesStats, this.nodeFilter);
    ...
    public Collection<Object> createComponents(
        ...
        this.anomalyDetectionIndices = new ADIndexManagement(
            client,
            clusterService,
            threadPool,
            settings,
        ...
        adStateManager = new ADNodeStateManager(
                client,
                xContentRegistry,
                settings,
                clientUtil,
        ...
        adSecurityClientUtil = new SecurityClientUtil(adStateManager, settings);

        double modelMaxSizePercent = AnomalyDetectorSettings.MODEL_MAX_SIZE_PERCENTAGE.get(settings);

        MemoryTracker adMemoryTracker = new MemoryTracker(
            jvmService,
            modelMaxSizePercent,
            clusterService,
        ...
        ADCheckpointDao checkpoint = new ADCheckpointDao(
            client,
            clientUtil,
        ...
        CacheProvider<ThresholdedRandomCutForest, ADNodeState, ADNodeStateManager, ADIndex, ADIndexManagement, ADCheckpointDao, ADCheckpointWriteWorker, ADCheckpointMaintainWorker, ADCacheBuffer, ADPriorityCache> adCacheProvider = new CacheProvider<>();

        CheckPointMaintainRequestAdapter<ThresholdedRandomCutForest, ADIndex, ADIndexManagement, ADCheckpointDao, ADPriorityCache> adAdapter = new CheckPointMaintainRequestAdapter<>(
                checkpoint,
                ADCommonName.CHECKPOINT_INDEX_NAME,
        ...
        ADCheckpointWriteWorker checkpointWriteQueue = new ADCheckpointWriteWorker(
                heapSizeBytes,
            ...
            ADCheckpointMaintainWorker checkpointMaintainQueue = new ADCheckpointMaintainWorker(
                heapSizeBytes,
        ...
        ADPriorityCache adPriorityCache =
                new ADPriorityCache(
                        checkpoint,
        ...
        ADEntityColdStart entityColdStarter = new ADEntityColdStart(
            getClock(),
            threadPool,
            adStateManager,
            AnomalyDetectorSettings.NUM_SAMPLES_PER_TREE,
        ...
        ADColdStartWorker coldstartQueue = new ADColdStartWorker(
            heapSizeBytes,
        ...
        ADModelManager modelManager = new ADModelManager(
            checkpoint,
            getClock(),
            AnomalyDetectorSettings.NUM_TREES,
            AnomalyDetectorSettings.NUM_SAMPLES_PER_TREE,
        ...
        ADIndexMemoryPressureAwareResultHandler multiEntityResultHandler = new ADIndexMemoryPressureAwareResultHandler(
            client,
            settings,
            threadPool,
            anomalyDetectionIndices,
        ...
        ADResultWriteWorker resultWriteQueue = new ADResultWriteWorker(
            heapSizeBytes,
        ...
        Map<String, TimeSeriesStat<?>> stats = ImmutableMap
            ...
            .put(StatNames.SINGLE_ENTITY_DETECTOR_COUNT.getName(), new TimeSeriesStat<>(true, new SettableSupplier()))
            .put(StatNames.MULTI_ENTITY_DETECTOR_COUNT.getName(), new TimeSeriesStat<>(true, new SettableSupplier()))
            ...
            .put(StatNames.MODEL_COUNT.getName(), new TimeSeriesStat<>(false, new ModelsOnNodeCountSupplier(modelManager, cacheProvider)))
            .put(StatNames.MODEL_CORRUTPION_COUNT.getName(), new TimeSeriesStat<>(false, new CounterSupplier()))
            .build();

        timeSeriesStats = new Stats(stats);

        ADCheckpointReadWorker checkpointReadQueue = new ADCheckpointReadWorker(
            heapSizeBytes,
        ...
        ADColdEntityWorker coldEntityQueue = new ADColdEntityWorker(
            heapSizeBytes,
            AnomalyDetectorSettings.ENTITY_FEATURE_REQUEST_SIZE_IN_BYTES,
        ...
        TimeSeriesResultBulkIndexingHandler anomalyResultBulkIndexHandler = new TimeSeriesResultBulkIndexingHandler(
            client,
            settings,
            threadPool,
            this.clientUtil,
        ...
        TimeSeriesResultIndexingHandler<AnomalyResult> anomalyResultHandler = new TimeSeriesResultIndexingHandler<AnomalyResult>(
            client,
            settings,
            threadPool,
        ...
        // forecast components
        ForecastIndexManagement forecastIndices = new ForecastIndexManagement(
            client,
            clusterService,
            threadPool,
            settings,
        ...
        forecastStateManager = new ForecastNodeStateManager(
                client,
                xContentRegistry,
                settings,
                clientUtil,
        ...
        return ImmutableList
            .of(
                ...
                new ScalingExecutorBuilder(
                    ...
                    Math.max(1, OpenSearchExecutors.allocatedProcessors(settings) / 8),
                    TimeValue.timeValueMinutes(10),
                    FORECAST_THREAD_POOL_PREFIX + FORECAST_BATCH_TASK_THREAD_POOL_NAME
                )
            );
    }
    ...
    public List<Setting<?>> getSettings() {
        ...
        List<Setting<?>> systemSetting = ImmutableList
            .of(
                ...
                // restful apis
                ForecastSettings.REQUEST_TIMEOUT,
                // resource constraint
                ...
                // Security
                ForecastSettings.FILTER_BY_BACKEND_ROLES
            );
