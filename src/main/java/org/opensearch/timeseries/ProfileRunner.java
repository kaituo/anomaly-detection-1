...
public abstract class ProfileRunner<TaskCacheManagerType extends TaskCacheManager, TaskTypeEnum extends TaskType, TaskClass extends TimeSeriesTask, IndexType extends Enum<IndexType> & TimeSeriesIndex, IndexManagementType extends IndexManagement<IndexType>, TaskProfileType extends TaskProfile<TaskClass>, TaskManagerType extends TaskManager<TaskCacheManagerType, TaskTypeEnum, TaskClass, IndexType, IndexManagementType>, ConfigProfileType extends ConfigProfile<TaskClass, TaskProfileType>, ProfileActionType extends ActionType<ProfileResponse>>
    ...
    protected final List<TaskTypeEnum> historicalConfigTaskTypes;
    protected int maxCategoricalFields;
    protected ProfileName taskProfile;
    protected TaskProfileRunner<TaskClass, TaskProfileType> taskProfileRunner;
    ...
    public ProfileRunner(
        ...
        List<TaskTypeEnum> historicalConfigTaskTypes,
        ...
        this.historicalConfigTaskTypes = historicalConfigTaskTypes;
    ...
    private void calculateTotalResponsesToWait(
        ...
        client.get(getConfigRequest, ActionListener.wrap(getConfigResponse -> {
            if (getConfigResponse != null && getConfigResponse.isExists()) {
                ...
                listener.onFailure(new OpenSearchStatusException(CommonMessages.FAIL_TO_FIND_CONFIG_MSG + configId, BAD_REQUEST));
            }
        }, exception -> {
            logger.error(CommonMessages.FAIL_TO_FIND_CONFIG_MSG + configId, exception);
            listener.onFailure(new OpenSearchStatusException(CommonMessages.FAIL_TO_FIND_CONFIG_MSG + configId, INTERNAL_SERVER_ERROR));
        }));
    }
            ...
            if (getResponse != null && getResponse.isExists()) {
                try (
                    ...
                    if (profilesToCollect.contains(ProfileName.COORDINATING_NODE)
                        ...
                        || profilesToCollect.contains(ProfileName.INIT_PROGRESS)
                        || profilesToCollect.contains(ProfileName.STATE)) {
                        profileModels(config, profilesToCollect, job, true, delegateListener);
                    }
                    if (profilesToCollect.contains(taskProfile)) {
     ...
     * @param modelInPriorityCache Whether the models are stored in priority cache. AD single stream models are stored in ModelManager.
     * Other models are stored in priority cache.
    ...
    protected void profileModels(
        ...
        Job job,
        boolean modelInPriorityCache,
        MultiResponsesDelegateActionListener<ConfigProfileType> listener
    ) {
        DiscoveryNode[] dataNodes = nodeFilter.getEligibleDataNodes();
        ProfileRequest profileRequest = new ProfileRequest(config.getId(), profiles, modelInPriorityCache, dataNodes);
        client.execute(profileAction, profileRequest, onModelResponse(config, profiles, job, modelInPriorityCache, listener));// get init
                                                                                                                              // progress
    }
    ...
    private ActionListener<ProfileResponse> onModelResponse(
        ...
        return ActionListener.wrap(profileResponse -> {
            ...
            if (profilesToCollect.contains(ProfileName.SHINGLE_SIZE)) {
            ...
            if (modelInPriorityCache
                && (profilesToCollect.contains(ProfileName.INIT_PROGRESS) || profilesToCollect.contains(ProfileName.STATE))) {
    ...
    public void getLatestHistoricalTaskProfile(
        ...
        taskManager.getAndExecuteOnLatestConfigTask(configId, null, null, historicalConfigTaskTypes, task -> {
            if (task.isPresent()) {
