...
public abstract class IndexManagement<IndexType extends Enum<IndexType> & TimeSeriesIndex> implements LocalNodeClusterManagerListener {
    ...
    public static String getConfigMappings() throws IOException {
        URL url = ADIndexManagement.class.getClassLoader().getResource(TimeSeriesSettings.ANOMALY_DETECTORS_INDEX_MAPPING_FILE);
    ...
    public static String getJobMappings() throws IOException {
        URL url = ADIndexManagement.class.getClassLoader().getResource(TimeSeriesSettings.ANOMALY_DETECTOR_JOBS_INDEX_MAPPING_FILE);
        return Resources.toString(url, Charsets.UTF_8);
    }
    ...
    private void updateSettingIfNecessary(GroupedActionListener<Void> delegateListeneer) {
        ...
        for (IndexType adIndex : updates) {
            ...
            if (adIndex.isJobIndex()) {
                updateJobIndexSettingIfNecessary(
                    ADIndex.JOB.getIndexName(),
                    indexStates.computeIfAbsent(adIndex, k -> new IndexState(k.getMapping())),
    ...
    private void shouldUpdateIndex(IndexType index, ActionListener<Boolean> thenDo) {
        ...
        if (index.isAlias()) {
            exists = doesAliasExists(clusterService, index.getIndexName());
        } else {
            exists = doesIndexExists(clusterService, index.getIndexName());
        }
        if (false == exists) {
            thenDo.onResponse(Boolean.FALSE);
            return;
    ...
    public abstract <T> void initCustomResultIndexAndExecute(String resultIndex, ExecutorFunction function, ActionListener<T> listener);
