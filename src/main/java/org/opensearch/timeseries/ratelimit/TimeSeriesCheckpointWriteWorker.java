...
public abstract class TimeSeriesCheckpointWriteWorker<NodeStateType extends ExpiringState> extends BatchWorker<CheckpointWriteRequest, BulkRequest, BulkResponse, NodeStateType> {
    private static final Logger LOG = LogManager.getLogger(TimeSeriesCheckpointWriteWorker.class);

    protected final TimeSeriesCheckpointDao checkpoint;
    protected final String indexName;
    protected final Duration checkpointInterval;

    public TimeSeriesCheckpointWriteWorker(String queueName, long heapSize, int singleRequestSize,
            ...
            TimeSeriesNodeStateManager<NodeStateType> timeSeriesNodeStateManager, TimeSeriesCheckpointDao checkpoint,
            String indexName, Duration checkpointInterval) {
     ...
     */
    public void write(TimeSeriesModelState<?> modelState, boolean forceWrite, RequestPriority priority) {
        ...
        if (modelState.getModel() != null) {
            String configId = modelState.getId();
            String modelId = modelState.getModelId();
            if (modelId == null || configId == null) {
                return;
            }

            nodeStateManager.getConfig(configId, onGetConfig(configId, modelId, modelState, priority));
        }
    }
    ...
    private ActionListener<Optional<? extends Config>> onGetConfig(
            ...
            return ActionListener.wrap(detectorOptional -> {
                ...
                Config detector = detectorOptional.get();
                try {
    ...
    public void writeAll(List<TimeSeriesModelState<? extends EntityModel<?>>> modelStates, String detectorId, boolean forceWrite, RequestPriority priority) {
        ActionListener<Optional<? extends Config>> onGetForAll = ActionListener.wrap(detectorOptional -> {
            ...
            try {
                ...
                for (TimeSeriesModelState<? extends EntityModel<?>> state : modelStates) {
        ...
        nodeStateManager.getConfig(detectorId, onGetForAll);
    }
}
