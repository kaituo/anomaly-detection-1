...
public abstract class BatchWorker<RequestType extends QueuedRequest, BatchRequestType, BatchResponseType> extends
     ...
     */
    protected abstract BatchRequestType toBatchRequest(List<RequestType> toProcess, String tenantId, String dataSourceId);
    ...
    protected void execute(Runnable afterProcessCallback, Runnable emptyQueueCallback) {
        ...
        if (false == toProcess.isEmpty()) {
            ...
            Map<BatchKey, List<RequestType>> requestsByTenantAndDataSource = new HashMap<>();
            for (RequestType request : toProcess) {
                BatchKey batchKey = new BatchKey(request.getTenantId(), request.getDataSourceId());
                requestsByTenantAndDataSource.computeIfAbsent(batchKey, k -> new ArrayList<>()).add(request);
            }

            AtomicInteger pendingBatches = new AtomicInteger(requestsByTenantAndDataSource.size());
            ...
            for (Map.Entry<BatchKey, List<RequestType>> entry : requestsByTenantAndDataSource.entrySet()) {
                BatchKey batchKey = entry.getKey();
                List<RequestType> partition = entry.getValue();
                BatchRequestType batchRequest = toBatchRequest(partition, batchKey.tenantId(), batchKey.dataSourceId());
                LOG
                    .info(
                        "Executing tenant batch for worker [{}], tenant [{}], dataSource [{}], requests [{}]",
                        workerName,
                        batchKey.tenantId(),
                        batchKey.dataSourceId(),
                        partition.size()
                    );
    ...
    private record BatchKey(String tenantId, String dataSourceId) {
    }
