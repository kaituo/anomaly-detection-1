    ...
    private void runJob(
        ...
        try (InjectSecurity injectSecurity = new InjectSecurity(configId, settings, client.threadPool().getThreadContext())) {
            ...
            client.execute(resultAction, request, ActionListener.wrap(response -> {
                indexResult(jobParameter, lockService, lock, detectionStartTime, executionStartTime, response, recorder, detector);
            },
                exception -> {
                    handleException(jobParameter, lockService, lock, detectionStartTime, executionStartTime, exception, recorder, detector);
                }
            ));
        } catch (Exception e) {
