...
public abstract class TaskManager<TaskCacheManagerType extends TaskCacheManager, TaskTypeEnum extends TaskType, TaskClass extends TimeSeriesTask, DataStoreType extends DelegatingDataManagement<? extends Enum<? extends TimeSeriesIndex>>> {
    ...
    public void updateLatestRealtimeTask(
        ...
        nodeStateManager.getJob(configId, tenantId, false, ActionListener.wrap(jobOptional -> {
            ...
            updateLatestRealtimeTaskWithRetry(configId, tenantId, updatedFields, MAX_UPDATE_LATEST_REALTIME_TASK_RETRIES, ActionListener.wrap(r -> {
     ...
     *
     * In multi-tenant coordinator mode, job start goes through EventBridge
     * ({@code EventBridgeHandler#startJob}) and does not create a realtime task document immediately.
     * On first execution, AD may recreate the missing realtime task
     * ({@code ADTaskManager#initRealtimeTaskCacheAndCleanupStaleCache}).
