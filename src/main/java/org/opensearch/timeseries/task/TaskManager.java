...
public abstract class TaskManager<TaskCacheManagerType extends TaskCacheManager, TaskTypeEnum extends TaskType, TaskClass extends TimeSeriesTask, DataStoreType extends DelegatingDataManagement<? extends Enum<? extends TimeSeriesIndex>>> {
    ...
    public void updateLatestTask(
        ...
        getAndExecuteOnLatestConfigLevelTask(configId, tenantId, taskTypes, (task) -> {
            if (task.isPresent()) {
                logger.debug("found latest realtime task for update config={} taskId={}", configId, task.get().getTaskId());
                updateTask(task.get().getTaskId(), updatedFields, tenantId, listener);
            } else {
                logger.debug("latest realtime task missing during update config={} tenant={}", configId, tenantId);
    ...
    public <T> void updateLatestFlagOfOldTasksAndCreateNewTask(
        ...
        TenantContext tenantContext = TenantContext.user(config);
        logger
            .debug(
                "Reset latest task flags submit config={} tenant={} dataSource={} stateIndex={} taskTypes={} query={} script={}",
                config.getId(),
                tenantContext.getTenantId(),
                tenantContext.getDataSourceId(),
                stateIndex,
                taskTypeToString(getTaskTypes(dateRange, runOnce)),
                query,
                script
            );
        dataAccess.updateByQuery(updateByQueryRequest, TenantContext.user(config), ActionListener.wrap(r -> {
            List<BulkItemResponse.Failure> bulkFailures = r.getBulkFailures();
            logger
                .debug(
                    "Reset latest task flags completed config={} tenant={} dataSource={} stateIndex={} updated={} bulkFailures={}",
                    config.getId(),
                    tenantContext.getTenantId(),
                    tenantContext.getDataSourceId(),
                    stateIndex,
                    r == null ? -1 : r.getUpdated(),
                    bulkFailures == null ? -1 : bulkFailures.size()
                );
            if (bulkFailures.isEmpty()) {
                createTaskAfterLatestFlagReset(config, dateRange, runOnce, user, initialState, listener);
            } else {
            ...
            if (ExceptionUtil.isIndexNotFoundInMessage(e) || ExceptionUtil.isSearchPhaseExecutionException(e)) {
                logger.error("Failed to reset latest task flags because state index is unavailable for config {}", config.getId(), e);
                listener.onFailure(e);
            } else {
    ...
    private <T> void createTaskAfterLatestFlagReset(
        Config config,
        DateRange dateRange,
        boolean runOnce,
        User user,
        ...
        // node as local node.
        String coordinatingNode = dateRange == null ? null : getLocalCoordinatingNodeIdForHistoricalTask();
    ...
    public <T> void getAndExecuteOnLatestTasks(
        ...
        List<String> taskTypeNames = taskTypes == null ? List.of() : TaskType.taskTypeToString(taskTypes);
        if (taskTypes != null && taskTypes.size() > 0) {
            query.filter(new TermsQueryBuilder(TimeSeriesTask.TASK_TYPE_FIELD, taskTypeNames));
        }
        ...
        logger
            .debug(
                "Latest task search submit config={} tenant={} stateIndex={} resetTaskState={} requestedSize={} taskTypes={} parentTaskId={} entity={} query={}",
                configId,
                tenantId,
                stateIndex,
                resetTaskState,
                ...
                taskTypeNames,
                parentTaskId,
                entity == null ? null : entity.getAttributes(),
                sourceBuilder
            );
        dataAccess.search(searchRequest, TenantContext.user(tenantId), ActionListener.wrap(r -> {
            ...
            while (iterator.hasNext()) {
                ...
                logger
                    .debug(
                        "Latest task search hit config={} tenant={} hitIndex={} hitId={} sourceTenant={} sourceTaskType={} sourceIsLatest={} sourceState={} sourceExecutionStart={} sourceLastUpdate={} sourceCoordinator={}",
                        configId,
                        tenantId,
                        searchHit.getIndex(),
                        searchHit.getId(),
                        source == null ? null : source.get(CommonName.TENANT_ID_FIELD),
                        source == null ? null : source.get(TimeSeriesTask.TASK_TYPE_FIELD),
                        source == null ? null : source.get(TimeSeriesTask.IS_LATEST_FIELD),
                        source == null ? null : source.get(TimeSeriesTask.STATE_FIELD),
                        source == null ? null : source.get(TimeSeriesTask.EXECUTION_START_TIME_FIELD),
                        source == null ? null : source.get(TimeSeriesTask.LAST_UPDATE_TIME_FIELD),
                        source == null ? null : source.get(TimeSeriesTask.COORDINATING_NODE_FIELD)
                    );
            ...
            if (resetTaskState) {
                logger.debug("Latest task search delegating to resetLatestConfigTaskState config={} tenant={}", configId, tenantId);
                resetLatestConfigTaskState(tsTasks, function, transportService, listener);
            } else {
                logger.debug("Latest task search invoking callback directly config={} tenant={}", configId, tenantId);
                function.accept(tsTasks);
            }
        }, e -> {
            if (ExceptionUtil.isIndexNotFoundInMessage(e)) {
                logger.error("Latest task search failed because state index is missing config={} tenant={}", configId, tenantId, e);
                listener.onFailure(e);
            } else if (ExceptionUtil.isSearchPhaseExecutionException(e)) {
                logger.error("Latest task search failed because state index is unavailable config={} tenant={}", configId, tenantId, e);
                listener.onFailure(e);
            } else {
                // unknown exceptions
    ...
    public void updateTask(String taskId, Map<String, Object> updatedFields, String tenantId, ActionListener<UpdateResponse> listener) {
        ...
        logger
            .debug(
                "Task update submit taskId={} tenant={} stateIndex={} refreshPolicy={} updatedFields={}",
                taskId,
                tenantId,
                stateIndex,
                updateRequest.getRefreshPolicy(),
    ...
    public <T> void createTaskDirectly(TaskClass tsTask, Consumer<IndexResponse> function, ActionListener<T> listener) {
        ...
        try (XContentBuilder builder = XContentFactory.jsonBuilder()) {
            ...
            TenantContext tenantContext = tenantContext(tsTask);
            logger
                .debug(
                    "Task create submit config={} tenant={} dataSource={} stateIndex={} taskType={} state={} isLatest={} refreshPolicy={}",
                    tsTask.getConfigId(),
                    tenantContext.getTenantId(),
                    tenantContext.getDataSourceId(),
                    stateIndex,
            ...
            dataAccess.index(request, tenantContext, ActionListener.wrap(r -> {
                logger
                    .debug(
                        "Task create completed config={} tenant={} dataSource={} stateIndex={} taskId={} result={} status={}",
                        tsTask.getConfigId(),
                        tenantContext.getTenantId(),
                        tenantContext.getDataSourceId(),
                        stateIndex,
                        r == null ? null : r.getId(),
                        r == null ? null : r.getResult(),
    ...
    private TenantContext tenantContext(TimeSeriesTask task) {
        if (task instanceof ADTask) {
            ADTask adTask = (ADTask) task;
            if (adTask.getDetector() != null) {
                return TenantContext.user(adTask.getDetector());
            }
        }
        if (task instanceof ForecastTask) {
            ForecastTask forecastTask = (ForecastTask) task;
            if (forecastTask.getForecaster() != null) {
                return TenantContext.user(forecastTask.getForecaster());
            }
        }
        return TenantContext.user(task.getTenantId());
    }
    ...
    protected <T> void cleanOldConfigTaskDocs(
        ...
        String configId = tsTask.getConfigId();
        deleteTaskDocs(configId, searchRequest, tenantContext(tsTask), () -> {
            if (tsTask.isHistoricalTask()) {
    ...
    public <T> void deleteTaskDocs(
        ...
        deleteTaskDocs(configId, searchRequest, TenantContext.user(tenantId), function, listener);
    }
    ...
    private <T> void deleteTaskDocs(
        ...
        ActionListener<SearchResponse> searchListener = ActionListener.wrap(r -> {
            ...
            if (iterator.hasNext()) {
                ...
                dataAccess.bulk(bulkRequest, tenantContext, ActionListener.wrap(res -> {
        ...
        dataAccess.search(searchRequest, tenantContext, searchListener);
    }
    ...
    public void resetLatestFlagAsFalse(List<TaskClass> tasks, String tenantId) {
        ...
        dataAccess.bulk(bulkRequest, tenantContext(tasks.get(0)), ActionListener.wrap(res -> {
            BulkItemResponse[] bulkItemResponses = res.getItems();
            if (bulkItemResponses != null && bulkItemResponses.length > 0) {
                for (BulkItemResponse bulkItemResponse : bulkItemResponses) {
                    if (!bulkItemResponse.isFailed()) {
