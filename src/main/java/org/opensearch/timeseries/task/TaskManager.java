...
public abstract class TaskManager<TaskCacheManagerType extends TaskCacheManager, TaskTypeEnum extends TaskType, TaskClass extends TimeSeriesTask, IndexType extends Enum<IndexType> & TimeSeriesIndex, IndexManagementType extends IndexManagement<IndexType>> {
    ...
    public TaskManager(
        ...
        this.maxOldAdTaskDocsPerConfig = maxOldADTaskDocsPerConfigSetting.get(settings);
        clusterService
            .getClusterSettings()
            .addSettingsUpdateConsumer(maxOldADTaskDocsPerConfigSetting, it -> maxOldAdTaskDocsPerConfig = it);
    ...
    protected <T> void resetLatestConfigTaskState(
        ...
        for (TimeSeriesTask task : tasks) {
            if (!task.isEntityTask() && !task.isDone()) {
                if (!task.isHistoricalTask()) {
    ...
    protected <T> void deleteTaskDocs(String configId, SearchRequest searchRequest, ExecutorFunction function, ActionListener<T> listener) {
            ...
            if (iterator.hasNext()) {
                ...
                while (iterator.hasNext()) {
                    ...
                    try (XContentParser parser = createXContentParserFromRegistry(xContentRegistry, searchHit.getSourceRef())) {
                        ...
                        bulkRequest.add(new DeleteRequest(DETECTION_STATE_INDEX).id(task.getTaskId()));
                    } catch (Exception e) {
                        listener.onFailure(e);
                    }
                }
