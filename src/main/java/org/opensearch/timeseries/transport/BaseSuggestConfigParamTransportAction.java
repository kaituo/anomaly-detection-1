...
public abstract class BaseSuggestConfigParamTransportAction extends
    ...
    protected void suggestInterval(Config config, User user, TimeValue timeout, ActionListener<Pair<IntervalTimeConfiguration, Map<String, Object>>> listener) {
        ...
        ActionListener<Pair<Optional<Long>, Map<String, Object>>> latestTimeListener = ActionListener.wrap(latestEntityAttributes -> {
            ...
            if (latestTime.isPresent()) {
                ...
                intervalCalculation.findInterval(ActionListener
                        .wrap(
    ...
    protected void suggestHistory(Config config, User user, TimeValue timeout, boolean suggestInterval,
        ...
        if (!suggestInterval && config.getInterval() != null) {
            ...
            HistorySuggest historySuggest = new HistorySuggest(config, user, searchFeatureDao, interval,
                 ...
                 // Additionally, it could lead to inefficiencies by requiring more processing cycles to analyze past data.
                    new HashMap<String, Object>(), clock);
            historySuggest.suggestHistory(ActionListener.wrap(
                    historyResponse -> listener.onResponse(new SuggestConfigParamResponse.Builder()
                            .history(historyResponse.getHistory()).interval(interval).build()),
                    listener::onFailure));
            return;
        }
        suggestInterval(config, user, timeout, ActionListener.wrap(intervalEntity -> {
            HistorySuggest historySuggest = new HistorySuggest(config, user, searchFeatureDao, intervalEntity.getLeft(),
                    intervalEntity.getRight(), clock);
            historySuggest.suggestHistory(ActionListener.wrap(
                    historyResponse -> listener.onResponse(new SuggestConfigParamResponse.Builder()
                            .history(historyResponse.getHistory()).interval(intervalEntity.getLeft()).build()),
                    listener::onFailure));
        }, listener::onFailure));

    }

    protected void suggestWindowDelay(Config config, User user, TimeValue timeout,
            ActionListener<SuggestConfigParamResponse> listener) {
        LatestTimeRetriever latestTimeRetriever = new LatestTimeRetriever(
                config,
                timeout,
        ...
        ActionListener<Pair<Optional<Long>, Map<String, Object>>> latestTimeListener = ActionListener.wrap(latestEntityAttributes -> {
            ...
            if (latestTime.isPresent()) {
                ...
                System.out.println("hello9:"+currentMillis+" "+latestTime.get());

                // in case windowDelayMillis is small, we want at least 1 minute
                listener.onResponse(new SuggestConfigParamResponse.Builder().windowDelay(new IntervalTimeConfiguration((long)Math.ceil(windowDelayMillis / 60000.0), ChronoUnit.MINUTES)).build());
            } else {
