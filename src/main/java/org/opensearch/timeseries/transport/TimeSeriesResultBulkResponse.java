...
public class TimeSeriesResultBulkResponse extends ActionResponse {
     ...
     */
    public TimeSeriesResultBulkResponse(List<IndexRequest> retryRequests) {
        this.retryRequests = retryRequests;
    }

    public TimeSeriesResultBulkResponse() {
        this.retryRequests = null;
    }

    public TimeSeriesResultBulkResponse(StreamInput in) throws IOException {
        int size = in.readInt();
        if (size > 0) {
