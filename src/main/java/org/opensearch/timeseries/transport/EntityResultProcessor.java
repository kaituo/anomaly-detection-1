 ...
 *
 */
public class EntityResultProcessor<RCFModelType extends ThresholdedRandomCutForest, NodeStateType extends NodeState, IntermediateResultType extends IntermediateResult, IndexType extends Enum<IndexType> & TimeSeriesIndex, IndexManagementType extends IndexManagement<IndexType>, IndexableResultType extends IndexableResult, ResultWriteRequestType extends ResultWriteRequest<IndexableResultType>, ResultWriteBatchRequestType extends ResultBulkRequest<IndexableResultType, ResultWriteRequestType>, CheckpointDaoType extends CheckpointDao<RCFModelType, IndexType, IndexManagementType>, CheckpointWriteWorkerType extends CheckpointWriteWorker<NodeStateType, RCFModelType, IndexType, IndexManagementType, CheckpointDaoType>, ModelColdStartType extends ModelColdStart<NodeStateType, RCFModelType, IndexType, IndexManagementType, CheckpointDaoType, CheckpointWriteWorkerType>, ModelManagerType extends ModelManager<RCFModelType, NodeStateType, IntermediateResultType, IndexType, IndexManagementType, CheckpointDaoType, CheckpointWriteWorkerType, ModelColdStartType>, CacheType extends Cache<RCFModelType>, ColdStartWorkerType extends ColdStartWorker<NodeStateType, RCFModelType, IndexType, IndexManagementType, CheckpointDaoType, CheckpointWriteWorkerType, ModelColdStartType, CacheType>, ResultHandlerType extends IndexMemoryPressureAwareResultHandler<ResultWriteBatchRequestType, ResultBulkResponse, IndexType, IndexManagementType>, ResultWriteWorkerType extends ResultWriteWorker<IndexableResultType, ResultWriteRequestType, ResultWriteBatchRequestType, NodeStateType, IndexType, IndexManagementType, ResultHandlerType>, HCCheckpointReadWorkerType extends CheckpointReadWorker<NodeStateType, RCFModelType, IndexableResultType, ResultWriteRequestType, ResultWriteBatchRequestType, IntermediateResultType, IndexType, IndexManagementType, CheckpointDaoType, CheckpointWriteWorkerType, ModelColdStartType, ModelManagerType, CacheType, ColdStartWorkerType, ResultHandlerType, ResultWriteWorkerType>, ColdEntityWorkerType extends ColdEntityWorker<NodeStateType, RCFModelType, IndexableResultType, ResultWriteRequestType, ResultWriteBatchRequestType, IndexType, IndexManagementType, CheckpointDaoType, IntermediateResultType, ModelManagerType, CheckpointWriteWorkerType, ModelColdStartType, CacheType, ColdStartWorkerType, ResultHandlerType, ResultWriteWorkerType, HCCheckpointReadWorkerType>> {

    private static final Logger LOG = LogManager.getLogger(EntityResultProcessor.class);

    private HCCacheProvider<RCFModelType> cache;
    ...
    public EntityResultProcessor(
        HCCacheProvider<RCFModelType> cache,
    ...
    public ActionListener<Optional<? extends Config>> onGetConfig(
        ...
        return ActionListener.wrap(configOptional -> {
            ...
            for (Entry<Entity, double[]> entityEntry : request.getEntities().entrySet()) {
                ...
                if (isEntityFromOldNodeMsg(entity) && config.getCategoryField() != null && config.getCategoryField().size() == 1) {
                    Map<String, String> attrValues = entity.getAttributes();
                    // handle a request from a version before OpenSearch 1.1.
                    entity = Entity.createSingleAttributeEntity(config.getCategoryField().get(0), attrValues.get(CommonName.EMPTY_FIELD));
                }
                ...
                try {
                    ...
                    stats.getStat(StatNames.MODEL_CORRUTPION_COUNT.getName()).increment();
