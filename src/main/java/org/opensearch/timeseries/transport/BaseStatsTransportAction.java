...
public abstract class BaseStatsTransportAction extends HandledTransportAction<StatsRequest, StatsTimeSeriesResponse> {
    ...
    protected Map<String, Object> getClusterStatsMap(StatsRequest statsRequest) {
        ...
        stats
            ...
            .forEach(s -> clusterStats.put(s.getKey(), s.getValue().getValueForTenant(statsRequest.getTenantId())));
        return clusterStats;
    }
    ...
    protected Map<String, Object> getNodeStatsMap(StatsRequest statsRequest) {
        ...
        stats
            ...
            .stream()
            .filter(s -> statsToBeRetrieved.contains(s.getKey()))
            .forEach(s -> nodeStats.put(s.getKey(), s.getValue().getValueForTenant(statsRequest.getTenantId())));
        return nodeStats;
    }
