...
public class TimeSeriesResultBulkIndexingHandler<T extends ToXContentObject> extends TimeSeriesResultIndexingHandler<T> {
    private static final Logger LOG = LogManager.getLogger(TimeSeriesResultBulkIndexingHandler.class);

    private TimeSeriesIndices timeseriesIndices;

    public TimeSeriesResultBulkIndexingHandler(
        Client client,
        Settings settings,
        ...
        super(client, settings, threadPool, indexName, timeseriesIndices, clientUtil, indexUtils, clusterService,
                backOffDelaySetting,
                maxRetrySetting);
     ...
     */
    public void bulkIndexAnomalyResult(String resultIndex, List<T> anomalyResults, String id, ActionListener<BulkResponse> listener) {
        ...
        try {
            if (resultIndex != null) {
                ...
                if (!timeseriesIndices.doesIndexExist(resultIndex)) {
                    throw new EndRunException(id, CommonMessages.CAN_NOT_FIND_RESULT_INDEX + resultIndex, true);
                }
                if (!timeseriesIndices.isValidResultIndexMapping(resultIndex)) {
                    throw new EndRunException(id, "wrong index mapping of custom AD result index", true);
                }
    ...
    private void bulkSaveDetectorResult(List<T> anomalyResults, ActionListener<BulkResponse> listener) {
        bulkSaveDetectorResult(indexName, anomalyResults, listener);
    }

    private void bulkSaveDetectorResult(String resultIndex, List<T> anomalyResults, ActionListener<BulkResponse> listener) {
