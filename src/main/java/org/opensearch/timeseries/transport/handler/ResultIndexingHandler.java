...
public class ResultIndexingHandler<ResultType extends IndexableResult, IndexType extends Enum<IndexType> & TimeSeriesIndex, DataManagementType extends DelegatingDataManagement<IndexType>> {
    ...
    public void index(ResultType toSave, String configId, String indexOrAliasName) {
        if (indexOrAliasName != null) {
            indexToCustomIndex(toSave, configId, indexOrAliasName);
        } else {
            indexToDefaultIndex(toSave, configId);
        }
    }

    private void indexToCustomIndex(ResultType toSave, String configId, String indexOrAliasName) {
        discoveryNodeSelector.hasIndicesBlock(ClusterBlockLevel.WRITE, new String[] { indexOrAliasName }, ActionListener.wrap(hasBlock -> {
            if (hasBlock) {
            ...
            LOG.error(String.format(Locale.ROOT, "Failed to check indices block for %s", indexOrAliasName), e);
        }));
    }
    ...
    private void proceedWithCustomIndex(ResultType toSave, String configId, String indexOrAliasName) {
        ...
        // create a new one.
        if (!dataManagement.doesResultIndexExists(indexOrAliasName, tenantId)
            && !dataManagement.doesResultAliasExists(indexOrAliasName, tenantId)) {
            dataManagement.initCustomResultIndexDirectly(indexOrAliasName, ActionListener.wrap(response -> {
                if (response.isAcknowledged()) {
                    ...
                    LOG
                        .error(
                            String
                                .format(Locale.ROOT, "Creating custom result index %s with mappings call not acknowledged", indexOrAliasName)
                        );
                }
            }, exception -> {
    ...
    private void indexToDefaultIndex(ResultType toSave, String configId) {
        discoveryNodeSelector
            .hasIndicesBlock(ClusterBlockLevel.WRITE, new String[] { this.defaultResultIndexName }, ActionListener.wrap(hasBlock -> {
                if (hasBlock) {
                    LOG.warn(String.format(Locale.ROOT, CANNOT_SAVE_ERR_MSG, configId));
                    return;
                }
                ...
                LOG.error(String.format(Locale.ROOT, "Failed to check indices block for %s", defaultResultIndexName), e);
            }));
    }
