...
public abstract class TimeSeriesResultBulkTransportAction<ResultBulkRequestType extends TimeSeriesResultBulkRequest<? extends ResultWriteRequest>> extends HandledTransportAction<ResultBulkRequestType, TimeSeriesResultBulkResponse> {
    private static final Logger LOG = LogManager.getLogger(TimeSeriesResultBulkTransportAction.class);
    ...
    public TimeSeriesResultBulkTransportAction(
            String actionName,
    ...
    @Override
    protected void doExecute(Task task, ResultBulkRequestType request, ActionListener<TimeSeriesResultBulkResponse> listener) {
        ...
        if (results == null || results.size() < 1) {
            listener.onResponse(new TimeSeriesResultBulkResponse());
        }
        ...
        if (bulkRequest.numberOfActions() > 0) {
            client.execute(BulkAction.INSTANCE, bulkRequest, ActionListener.wrap(bulkResponse -> {
                ...
                listener.onResponse(new TimeSeriesResultBulkResponse(failedRequests));
            }, e -> {
