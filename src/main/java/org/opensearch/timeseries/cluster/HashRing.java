...
public class HashRing {
    ...
    /**
     * Return the local node identity used by the hash ring.
     *
     * Single-tenant deployments keep using the OpenSearch node ID. Multi-tenant deployments
     * can use CloudMap-style node IDs, so this method exposes the same resolution that hash-ring
     * routing uses internally.
     *
     * @return local hash-ring node ID
     */
    public String getLocalNodeIdForHashRing() {
        return resolveLocalNodeVersionKey(clusterService.localNode());
    }
