...
public class HashRing {
    ...
    public void buildCirclesForRealtime() {
         ...
         * without a fresh node-change event. Before this diff, realtime rebuild attempts only
         * entered through queued nodeChangeEvents. After this diff, a due deferred probe is also
         * enough to re-enter buildCirclesForRealtime(), which lets newly discovered but not-yet-
         * ready nodes be retried later without requiring another membership callback.
         */
        if (nodeChangeEvents.isEmpty() && !hasDueDeferredNodeProbe()) {
            return;
        }
        buildCircles(
