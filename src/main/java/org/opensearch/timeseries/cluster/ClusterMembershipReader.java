...
public final class ClusterMembershipReader implements AutoCloseable, Runnable {
    ...
    private volatile DiscoveryNodes cachedDiscoveryNodes = DiscoveryNodes.builder().build();
    ...
    private CompletionStage<Long> readLatestRevisionIdOnly() {
        QueryRequest request = QueryRequest.builder()
            ...
            .expressionAttributeValues(Map.of(
                ":pk", AttributeValue.fromS(SERVICE_PK_PREFIX + service)
            ))
    ...
    private CompletionStage<ServiceRevisionData> conditionallyReadFullData(long remoteRevisionId) {
        ...
        if (remoteRevisionId == localRevisionId && remoteRevisionId != -1L) {
            LOG.debug("Revision {} unchanged, using cached data with {} tasks",
                remoteRevisionId, cachedDiscoveryNodes.getSize());
            return CompletableFuture.completedFuture(
                new ServiceRevisionData(remoteRevisionId, false, List.of())
            );
        }
        ...
        if (remoteRevisionId == -1L) {
            ...
            return CompletableFuture.completedFuture(
                new ServiceRevisionData(-1L, localRevisionId != -1L, List.of())
            );
        }
    ...
    private void updateLocalState(long revisionId, List<String> tasks) {
        // Create DiscoveryNodes.Delta based on the difference between cachedTasks and input tasks
        if (hashRing != null) {
            try {
                DiscoveryNodes.Delta delta = createDeltaFromTaskChanges(tasks);
                if (delta.added() || delta.removed()) {
                    ...
                    hashRing.buildCircles(delta, ActionListener.runAfter(ActionListener.wrap(hasRingBuildDone -> {
                        ...
                        LOG.error("Failed updating hash ring from membership changes", e);
                    }), () -> {
    ...
    /**
     * Create a DiscoveryNodes.Delta based on changes between old and new task lists.
     * Tasks are IP addresses, so we need to map them to DiscoveryNode objects.
     */
    private DiscoveryNodes.Delta createDeltaFromTaskChanges(List<String> newTasks) {
        Builder newDiscoveryNodes = DiscoveryNodes.builder();
        for (String ipAddress : newTasks) {
            newDiscoveryNodes.add(TransportUtil.createDiscoveryNodeFromIp(ipAddress));
        }

        // Create DiscoveryNodes.Delta by calling delta(old).
        return newDiscoveryNodes.build().delta(cachedDiscoveryNodes);
    }
