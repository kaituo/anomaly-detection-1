        ...
        if (multiTenancyEnabled) {
            tasks = List
                .of(
                    ...
                    new ADSQSConsumerTask(nodeStateManager, hashRing, adCheckpointStore), // SQS consumer
                    new ADClusterMembershipReaderTask(hashRing) // ad cluster membership reader
                );
        } else {
            tasks = List.of(new ClusterManagerEventListener(client, adCheckpointStore, forecastCheckpointStore)); // checkpoint + hourly jobs
        }
