...
public class ClusterManagerEventListener implements ClusterManagerTask {
    ...
    public ClusterManagerEventListener(Client client, RunContext runContext) {
    ...
    public void init(
        ...
        this.clock = clock;
        this.dataAccess = Objects.requireNonNull(dataAccess, "dataAccess must not be null");
    ...
    private Cancellable scheduleAdCheckpointRetention() {        
        IndexCleanup indexCleanup = new IndexCleanup(client, dataAccess, clusterService, runContext);
        Runnable retention = new ADCheckpointIndexRetention(adCheckpointTtlDuration, clock, indexCleanup);

        return threadPool.scheduleWithFixedDelay(retention, TimeValue.timeValueHours(24), executorName());
    }
