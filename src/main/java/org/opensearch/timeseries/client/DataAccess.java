 ...
 * <p>
 * {@code tenantContext} carries tenant identity (used for isolation, header
 * propagation, audit). Data-plane endpoint routing (which {@code dataSourceId}
 * to talk to) is selected by {@link #bindRouting(String, String)} for the
 * synchronous portion of any data-plane call -- callers wrap their invocation
 * in a try-with-resources around the returned {@link Releasable}.
 */
public interface DataAccess {

    /**
     * Bind a {@code (tenantId, dataSourceId)} pair as the current data-plane routing intent
     * on this thread. The returned {@link Releasable} restores the previous binding when
     * closed; use it as a try-with-resources around the synchronous portion of a data-plane
     * call.
     * <p>
     * Implementations that don't talk to a remote data plane (e.g. local-cluster transport)
     * may return a no-op.
     *
     * @param tenantId tenant id; may be null in single-tenant mode
     * @param dataSourceId data source id used to resolve the data-plane endpoint
     * @return releasable scope that restores the previous routing binding on close
     */
    default Releasable bindRouting(String tenantId, String dataSourceId) {
        return () -> {};
    }
