...
public class DefaultDataAccess implements DataAccess {
    ...
    public DefaultDataAccess(
        ...
        this.indexNameExpressionResolver = Objects
            .requireNonNull(indexNameExpressionResolver, "indexNameExpressionResolver must not be null");
        this.configDocumentStore = new TransportConfigDocumentStore(this.client);
    }
    ...
    public void getFieldMappings(
        ...
        ActionListener<GetFieldMappingsResponse> getMappingResponseListener = ActionListener
            .wrap(response -> listener.onResponse(FieldMappingsView.from(response)), listener::onFailure);
        clientUtil
    ...
    private boolean isConfigSearch(SearchRequest request) {
        ...
        return indices != null && indices.length > 0 && java.util.Arrays.stream(indices).allMatch(this::isConfigIndex);
    }
