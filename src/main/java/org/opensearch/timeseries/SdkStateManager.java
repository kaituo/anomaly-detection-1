...
public class SdkStateManager implements StateManager {
    ...
    public SdkStateManager(
        ...
        this.sdkClient = initializeSdkClient(settings);
    ...
    private SdkClient initializeSdkClient(Settings settings) {
        ...
        try {
            ...
            LOG.warn("Failed to initialize remote metadata SDK client; falling back to index-backed state management.", e);
            throw e;
        }
    }
    ...
    private void fetchConfig(String configId, AnalysisType analysisType, String tenantId, ActionListener<GetResponse> listener) {
        GetDataObjectRequest request = GetDataObjectRequest
            .builder()
            .index(resolveConfigIndexName(analysisType))
            .id(configId)
            .tenantId(tenantId)
            .build();
        sdkClient.getDataObjectAsync(request).whenComplete((response, throwable) -> {
            if (throwable != null) {
                listener.onFailure(SdkClientUtils.unwrapAndConvertToException(throwable));
                return;
            }
            try {
                listener.onResponse(GetResponse.fromXContent(response.parser()));
            } catch (IOException e) {
                LOG.error("Failed to parse remote metadata get response for config {}", configId, e);
                listener
                    .onFailure(new OpenSearchStatusException("Failed to parse config " + configId, RestStatus.INTERNAL_SERVER_ERROR, e));
            }
        });
    }
    ...
    public void getJob(String configID, String tenantId, boolean cache, ActionListener<Optional<Job>> listener) {
        ...
        try {
            Optional<Job> jobFromSchedule = eventBridgeHandler.getJobFromSchedule(tenantId, configID);
