...
public class TimeSeriesException extends RuntimeException {
    ...
    public TimeSeriesException cloneWithMsgPrefix(String msgPrefix) {
        ...
        try {
            ...
            java.lang.reflect.Constructor<? extends TimeSeriesException> ctor =
                cls.getConstructor(String.class, String.class, Throwable.class);

            TimeSeriesException newEx = ctor.newInstance(this.getConfigId(), newMessage, this.getCause());
            copyExceptionDetails(this, newEx);
            return newEx;
        } catch (NoSuchMethodException e) {
            throw new IllegalStateException("Subclass " + this.getClass().getName()
                + " must provide a (String configId, String message, Throwable cause) constructor.", e);
        } catch (Exception e) {
