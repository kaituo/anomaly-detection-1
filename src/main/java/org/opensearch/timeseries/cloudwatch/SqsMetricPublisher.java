...
public class SqsMetricPublisher extends AbstractLifecycleComponent {
    ...
    public static SqsMetricPublisher createIfEnabled(Settings settings, ThreadPool threadPool, JobQueueAccountIdProvider accountProvider) {
        ...
        String serviceName = coordinatorSqsMetricServiceName(settings);
        ...
        if (clusterName == null
            ...
            LOG
                .warn(
                    "CloudWatch SQS metric publishing is enabled but cluster_name, service_name for coordinator, region, "
        ...
        String scheduleManagementExternalId = settings.get(AnomalyDetectorSettings.EVENT_BRIDGE_SCHEDULE_MANAGEMENT_EXTERNAL_ID.getKey());
        scheduleManagementExternalId = scheduleManagementExternalId == null ? null : scheduleManagementExternalId.trim();
    ...
    static String coordinatorSqsMetricServiceName(Settings settings) {
        /*
         * TotalSqsBacklogDeletedPressure is only consumed by the coordinator service auto scaling policy.
         * This publisher runs on master because master has the cluster-level scheduler/SQS view and can aggregate
         * the queue backlog/deleted signal once, instead of having every coordinator publish the same scaling signal.
         *
         * ECS target tracking matches this custom CloudWatch metric by exact dimensions. If we reused the local
         * node service name here, the master publisher would emit ServiceName=master-onepod/master-fleet and the
         * coordinator auto scaling policy would not match the metric.
         */
        String configuredServiceName = TimeSeriesSettings.cloudWatchMetricsServiceName(settings, TimeSeriesSettings.COORDINATOR_ROLE);
        if (configuredServiceName == null || configuredServiceName.isBlank()) {
            return configuredServiceName;
        }
        return configuredServiceName.toLowerCase(Locale.ROOT).contains("onepod") ? "coordinator-onepod" : "coordinator-fleet";
    }
    ...
    private CloudWatchClient createSourceCloudWatchClient(String accountId) {
        ...
        return doPrivileged(
            () -> CloudWatchClient
                ...
                .credentialsProvider(
                    SecurityUtil.createAssumeRoleCredentialsProvider(region, roleArn, "ad-sqs-cw-" + accountId, scheduleManagementExternalId)
                )
                .build()
        );
    }
