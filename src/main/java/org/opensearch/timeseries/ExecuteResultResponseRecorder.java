...
public abstract class ExecuteResultResponseRecorder<IndexType extends Enum<IndexType> & TimeSeriesIndex, DataManagementType extends DelegatingDataManagement<IndexType>, TaskCacheManagerType extends TaskCacheManager, TaskTypeEnum extends TaskType, TaskClass extends TimeSeriesTask, TaskManagerType extends TaskManager<TaskCacheManagerType, TaskTypeEnum, TaskClass, DataManagementType>, IndexableResultType extends IndexableResult> {
    ...
    protected void delayedUpdate(ResultResponse<IndexableResultType> response, String configId, String tenantId, Clock clock) {
        ...
        Runnable profileInitProgress = () -> DataPlaneClientFactoryContext.runWithRestoredContext(restorableContext, () -> {
            nodeStateManager.getConfig(configId, tenantId, analysisType, false, ActionListener.wrap(configOptional -> {
                DataPlaneClientFactoryContext.runWithRestoredContext(restorableContext, () -> {
                    ...
                    if (config.isLongFrequency()) {
                        log.debug("Update latest realtime task for long-interval config {}", configId);
                        ...
                        }, e -> DataPlaneClientFactoryContext.runWithRestoredContext(restorableContext, () -> {
                            log.error("Failed to update latest realtime task for " + configId, e);
                        })));
                    }
                });
            }, e -> DataPlaneClientFactoryContext.runWithRestoredContext(restorableContext, () -> log.warn("fail to get config", e))));
        });
    ...
    protected void updateLatestRealtimeTask(
        ...
        hasRecentResult(configId, tenantId, configIntervalInMinutes, error, clock, ActionListener.wrap(r -> {
            logRealtimeTaskUpdate(configId, tenantId, rcfTotalUpdates, configIntervalInMinutes, error, r);
            taskManager
            ...
            logRealtimeTaskUpdate(configId, tenantId, rcfTotalUpdates, configIntervalInMinutes, error, false);
            taskManager
                .updateLatestRealtimeTaskOnCoordinatingNode(
                    configId,
                    tenantId,
    ...
    private void logRealtimeTaskUpdate(
        String configId,
        String tenantId,
        Long rcfTotalUpdates,
        Long configIntervalInMinutes,
        String error,
        Boolean hasRecentResult
    ) {
        log
            .info(
                "updateLatestRealtimeTaskOnCoordinatingNode config={} tenant={} hasRecentResult={} rcfTotalUpdates={} intervalMinutes={} error={}",
                configId,
                tenantId,
                hasRecentResult,
                rcfTotalUpdates,
    ...
    public void indexResultException(
        ...
        try {
            ...
            resultHandler.index(resultToSave, configId, resultIndexOrAlias, config.getTenantId(), config.getDataSourceId());
    ...
    private void hasRecentResult(
        ...
        nodeStateManager.getConfig(configId, tenantId, analysisType, false, ActionListener.wrap(configOptional -> {
            ...
            log.error("hasRecentResult failed for config " + configId, e);
            listener.onFailure(new TimeSeriesException(configId, "fail to check recent result status", e));
        }));
    }
