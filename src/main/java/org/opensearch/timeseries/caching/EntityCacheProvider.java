/*
 * SPDX-License-Identifier: Apache-2.0
 *
 * The OpenSearch Contributors require contributions made to
 * this file be licensed under the Apache-2.0 license or a
 * compatible open source license.
 *
 * Modifications Copyright OpenSearch Contributors. See
 * GitHub history for details.
 */

package org.opensearch.timeseries.caching;

import org.opensearch.common.inject.Provider;
import org.opensearch.timeseries.ml.EntityModel;
import org.opensearch.timeseries.ml.TimeSeriesModelState;

/**
 * A wrapper to call concrete implementation of caching.  Used in transport
 * action.  Don't use interface because transport action handler constructor
 * requires a concrete class as input.
 *
 */
public class EntityCacheProvider<RCFModelType, ModelState extends TimeSeriesModelState<EntityModel<RCFModelType>>> implements Provider<EntityCache<RCFModelType, ModelState>> {
    private EntityCache<RCFModelType, ModelState> cache;

    public EntityCacheProvider() {

    }

    @Override
    public EntityCache<RCFModelType, ModelState> get() {
        return cache;
    }

    public void set(EntityCache<RCFModelType, ModelState> cache) {
        this.cache = cache;
    }
}
