...
public abstract class PriorityCache<RCFModelType extends ThresholdedRandomCutForest, IndexType extends Enum<IndexType> & TimeSeriesIndex, DataManagementType extends DelegatingDataManagement<IndexType>, CheckpointDaoType extends CheckpointDaoInterface<RCFModelType>, CheckpointWriterType extends CheckpointWriteWorker<RCFModelType, IndexType, DataManagementType, CheckpointDaoType>, CheckpointMaintainerType extends CheckpointMaintainWorker, CacheBufferType extends CacheBuffer<RCFModelType, IndexType, DataManagementType, CheckpointDaoType, CheckpointWriterType, CheckpointMaintainerType>>
    ...
    public PriorityCache(
        ...
        this.checkpointIntervalMins = toCheckpointIntervalMinutes(DateUtils.toDuration(checkpointSavingFreq.get(settings)));
        clusterService.getClusterSettings().addSettingsUpdateConsumer(checkpointSavingFreq, it -> {
            this.checkpointIntervalMins = toCheckpointIntervalMinutes(DateUtils.toDuration(it));
    ...
    public ModelState<RCFModelType> get(String modelId, Config config) {
        ...
        if (!maintenanceLock.isLocked() && modelState == null) {
            ...
            try {
                ModelState<RCFModelType> state = inActiveEntities
                    .get(inactiveEntityKey(tenantId, configId, modelId), new Callable<ModelState<RCFModelType>>() {
                        @Override
    ...
    private Optional<ModelState<RCFModelType>> getStateFromInactiveEntityCache(String tenantId, String configId, String modelId) {
        ...
        return Optional.ofNullable(inActiveEntities.getIfPresent(inactiveEntityKey(tenantId, configId, modelId)));
    }
    ...
    public boolean hostIfPossible(Config config, ModelState<RCFModelType> toUpdate) {
        ...
        Optional<ModelState<RCFModelType>> state = getStateFromInactiveEntityCache(config.getTenantId(), configId, modelId);
        ModelState<RCFModelType> modelState = null;
        if (state.isPresent()) {
    ...
    private void addIntoInactiveCache(ModelState<RCFModelType> removed) {
        ...
        if (Strings.isEmpty(removed.getConfigId()) || Strings.isEmpty(removed.getModelId())) {
            return;
        }
        inActiveEntities.put(inactiveEntityKey(removed.getTenantId(), removed.getConfigId(), removed.getModelId()), removed);
    }
    ...
    private void addEntity(List<Entity> destination, Entity entity, String tenantId, String configId) {
        ...
        if (entity != null) {
            ...
            if (modelId.isPresent() && inActiveEntities.getIfPresent(inactiveEntityKey(tenantId, configId, modelId.get())) != null) {
                destination.add(entity);
            }
        }
    }

    private String inactiveEntityKey(String tenantId, String configId, String modelId) {
        return StringUtil.getCompositeKey(StringUtil.getCompositeKey(tenantId, configId), modelId);
    }
    ...
    public Pair<List<Entity>, List<Entity>> selectUpdateCandidate(Collection<Entity> cacheMissEntities, String configId, Config config) {
        ...
        while (cacheMissEntitiesIter.hasNext()) {
            ...
            Optional<ModelState<RCFModelType>> state = getStateFromInactiveEntityCache(config.getTenantId(), configId, modelId.get());
        ...
        while (cacheMissEntitiesIter.hasNext()) {
            ...
            Optional<ModelState<RCFModelType>> inactiveState = getStateFromInactiveEntityCache(
                config.getTenantId(),
                configId,
                modelId.get()
            );
    ...
    public void clear(String tenantId, String configId) {
        ...
        int activeEntities = buffer == null ? 0 : buffer.getActiveEntities();
        if (buffer != null) {
            buffer.clear();
        }
        AtomicInteger inactiveEntities = new AtomicInteger();
        inActiveEntities.asMap().entrySet().removeIf(entry -> {
            ModelState<RCFModelType> state = entry.getValue();
            boolean shouldRemove = configId.equals(state.getConfigId()) && Objects.equals(tenantId, state.getTenantId());
            if (shouldRemove) {
                inactiveEntities.incrementAndGet();
            }
            return shouldRemove;
        });
        ...
        LOG
            .info(
                "Cleared priority cache for config [{}], tenant [{}], activeEntities [{}], inactiveEntities [{}]",
                configId,
                tenantId,
                activeEntities,
                inactiveEntities.get()
            );
    }
    ...
    public long getLastActiveTime(String tenantId, String detectorId, String entityModelId) {
        ...
        ModelState<RCFModelType> stateInActive = inActiveEntities.getIfPresent(inactiveEntityKey(tenantId, detectorId, entityModelId));
    ...
    private void setCheckpointFreqListener() {
        activeEnities.values().stream().forEach(cacheBuffer -> cacheBuffer.setCheckpointIntervalMins(checkpointIntervalMins));
    }

    private static int toCheckpointIntervalMinutes(Duration checkpointInterval) {
        long intervalMillis = checkpointInterval.toMillis();
        long intervalMinutes = Math.max(1L, (intervalMillis + TimeUnit.MINUTES.toMillis(1) - 1) / TimeUnit.MINUTES.toMillis(1));
    ...
    public void stopModel(String tenantId, String configId, String modelId) {
        ...
        inActiveEntities.invalidate(inactiveEntityKey(tenantId, configId, modelId));
        String key = StringUtil.getCompositeKey(tenantId, configId);
        CacheBufferType buffer = activeEnities.get(key);
        if (buffer != null) {
