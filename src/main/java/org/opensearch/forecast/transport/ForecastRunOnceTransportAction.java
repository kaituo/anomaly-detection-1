        ...
        client.search(request, ActionListener.wrap(searchResponse -> {
            ...
            if (hits.getTotalHits().value > 0) {
                ...
                if (attempt < MAX_RETRIES) {
                    ...
                    threadPool.schedule(() -> performSearchWithRetry(forecastID, taskId, request, attempt + 1, exceptionMsg),
                            TimeValue.timeValueMillis(delayMillis), FORECAST_THREAD_POOL_NAME);
                } else {
            ...
            if (attempt < MAX_RETRIES) {
                ...
                threadPool.schedule(() -> performSearchWithRetry(forecastID, taskId, request, attempt + 1, ExceptionUtil.getErrorMessage(e)),
                        TimeValue.timeValueMillis(delayMillis), FORECAST_THREAD_POOL_NAME);
            } else {
                updateTask(forecastID, taskId, TaskState.INIT_TEST_FAILED, ExceptionUtil.getErrorMessage(e));
            }
