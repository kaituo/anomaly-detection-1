...
public class IndexForecasterTransportAction extends HandledTransportAction<IndexForecasterRequest, IndexForecasterResponse> {
    ...
    protected void forecastExecute(
        ...
        stampTenantMetadata(forecaster, currentForecaster, request.getTenantId());
        RestRequest.Method method = request.getMethod();
        ...
        checkIndicesAndExecute(forecaster.getIndices(), request.getTenantId(), forecaster.getDataSourceId(), () -> {
    ...
    private void stampTenantMetadata(Forecaster forecaster, Forecaster currentForecaster, String tenantId) {
        forecaster.setTenantId(tenantId);
        if (forecaster.getDataSourceId() == null && currentForecaster != null) {
            forecaster.setDataSourceId(currentForecaster.getDataSourceId());
        }
        if (Strings.isBlank(tenantId)) {
            return;
        }
        TenantComponents tenantComponents = TenantAwareHelper.parseTenantId(tenantId);
        forecaster.setApplicationId(tenantComponents.applicationId());
        forecaster.setWorkspaceId(tenantComponents.workspaceId());
    }
    ...
    private void checkIndicesAndExecute(
        ...
        dataAccess
            .search(searchRequest, TenantContext.user(tenantId, dataSourceId), ActionListener.wrap(r -> { function.execute(); }, e -> {
