...
import static org.opensearch.forecast.settings.ForecastSettings.FILTER_BY_BACKEND_ROLES;
import static org.opensearch.forecast.util.ForecastParseUtils.checkFilterByBackendRoles;
import static org.opensearch.forecast.util.ForecastParseUtils.getForecaster;
import static org.opensearch.forecast.util.ForecastParseUtils.getUserContext;
...
public class IndexForecasterTransportAction extends HandledTransportAction<IndexForecasterRequest, IndexForecasterResponse> {
    ...
    public IndexForecasterTransportAction(
        ...
        filterByEnabled = ForecastSettings.FILTER_BY_BACKEND_ROLES.get(settings);
    ...
    protected void doExecute(Task task, IndexForecasterRequest request, ActionListener<IndexForecasterResponse> actionListener) {
        ...
        try (ThreadContext.StoredContext context = client.threadPool().getThreadContext().stashContext()) {
            resolveUserAndExecute(user, forecasterId, method, listener, (forecaster) -> forecastExecute(request, user, forecaster, context, listener));
        } catch (Exception e) {
            LOG.error(e);
    ...
    private void resolveUserAndExecute(
        ...
        try {
            ...
            if (filterByBackendRole && !checkFilterByBackendRoles(requestedUser, listener)) {
                return;
            }
            if (method == RestRequest.Method.PUT) {
                ...
                getForecaster(requestedUser, forecasterId, listener, function, client, clusterService, xContentRegistry, filterByBackendRole);
            } else {
    ...
    protected void forecastExecute(
        ...
        checkIndicesAndExecute(forecaster.getIndices(), () -> {
            ...
            IndexForecasterActionHandler indexAnomalyDetectorActionHandler = new IndexForecasterActionHandler(
                clusterService,
                client,
                clientUtil,
                transportService,
