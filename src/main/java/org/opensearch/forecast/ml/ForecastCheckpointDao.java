...
public class ForecastCheckpointDao extends CheckpointDao<RCFCaster, ForecastIndex, ForecastIndexManagement> {
    ...
    private Sample loadLastProcessedSample(Map<String, Object> checkpoint, String modelId) {
        Map<String, Object> lastProcessedSample = (Map<String, Object>) checkpoint.get(LAST_PROCESSED_SAMPLE_FIELD);
        if (lastProcessedSample == null || lastProcessedSample.size() == 0) {
            return null;
        }

        try {
            return Sample.extractSample(checkpoint);
        } catch (Exception e) {
