...
public class ForecastColdStart extends
    ...
    protected List<ForecastResult> trainModelFromDataSegments(
        ...
        List<AnomalyDescriptor> descriptors = caster.processSequentially(sequentialData, x -> true);

        System.out.println("hello80:"+descriptors.size());
        if (descriptors.size() != sequentialTime.size()) {
            logger.warn(new ParameterizedMessage(
                    "processSequentially returns different size than expected, got [{}], expecting [{}].",
                    descriptors.size(), sequentialTime.size()));
            return null;
        }
        ...
        for (int i = 0; i < descriptors.size(); i++) {
            ...
            List<ForecastResult> resultI = casterResult.toIndexableResults(config, time.getLeft(), time.getRight(),
                    now, now, ParseUtils.getFeatureData(dataValue, config), modelState.getEntity(),
                    resultMappingVersion, modelState.getModelId(), taskId, null);
            res.addAll(resultI);
