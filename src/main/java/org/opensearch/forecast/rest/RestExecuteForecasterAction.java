...
public class RestExecuteForecasterAction extends BaseRestHandler {
    ...
    private final Logger logger = LogManager.getLogger(RestExecuteForecasterAction.class);

    public RestExecuteForecasterAction(Settings settings, ClusterService clusterService) {
    ...
    private String validateAdExecutionInput(ForecasterExecutionInput input) {
        if (StringUtils.isBlank(input.getForecasterId())) {
            return "Must set forecaster id or detector";
        }
