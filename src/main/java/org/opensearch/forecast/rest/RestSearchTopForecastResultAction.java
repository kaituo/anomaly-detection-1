...
public class RestSearchTopForecastResultAction extends BaseRestHandler {
    ...
    private static final String URL_PATH = String
        .format(
            Locale.ROOT,
            "%s/{%s}/%s/%s",
            TimeSeriesAnalyticsPlugin.FORECAST_BASE_URI,
    ...
    private SearchTopForecastResultRequest getSearchTopForecastResultRequest(RestRequest request) throws IOException {
        ...
        return SearchTopForecastResultRequest.parse(parser, forecasterId, historical);
    }
    ...
    public List<Route> routes() {
        return ImmutableList
            .of(
                // new Route(RestRequest.Method.POST, URL_PATH), new Route(RestRequest.Method.GET, URL_PATH)
            );
    }
}
