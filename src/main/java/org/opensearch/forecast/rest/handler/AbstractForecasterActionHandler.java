 ...
 *
 * @param <T> the response type returned by the handler
 */
public abstract class AbstractForecasterActionHandler<T extends ActionResponse> extends
    ...
    protected Config copyConfig(User user, Config config) {
        Forecaster copiedForecaster = new Forecaster(
            config.getId(),
            config.getVersion(),
            config.getName(),
        ...
        copiedForecaster.setApplicationId(config.getApplicationId());
        copiedForecaster.setDataSourceId(config.getDataSourceId());
        copiedForecaster.setWorkspaceId(config.getWorkspaceId());
        copiedForecaster.setS3Reference(config.getS3Reference());
        return copiedForecaster;
    }
    ...
    protected Forecaster createForecaster(Forecaster forecaster, User user) {
        Forecaster createdForecaster = new Forecaster(
            forecaster.getId(),
        ...
        createdForecaster.setApplicationId(forecaster.getApplicationId());
        createdForecaster.setDataSourceId(forecaster.getDataSourceId());
        createdForecaster.setWorkspaceId(forecaster.getWorkspaceId());
        createdForecaster.setS3Reference(forecaster.getS3Reference());
        return createdForecaster;
    }
}
