...
public class RestForecasterSuggestAction extends BaseRestHandler {
    ...
    private Boolean suggestTypesAreAccepted(String suggestType) {
        Set<String> typesInRequest = new HashSet<>(Arrays.asList(suggestType.split(",")));
        // only support interval suggest now
        return (!Collections.disjoint(typesInRequest, Set.of(Forecaster.FORECAST_INTERVAL_FIELD)));
    }
    ...
    protected BaseRestHandler.RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
        ...
        try {
            ...
            if (!StringUtils.isBlank(typesStr)) {
                if (!suggestTypesAreAccepted(typesStr)) {
