...
public class Forecaster extends Config {
    ...
    public Forecaster(String forecasterId, Long version, String name, String description, String timeField,
            List<String> indices, List<Feature> features, QueryBuilder filterQuery, TimeConfiguration forecastInterval,
            TimeConfiguration windowDelay, Integer shingleSize, Map<String, Object> uiMetadata, Integer schemaVersion,
            Instant lastUpdateTime, List<String> categoryFields, User user, String resultIndex, Integer horizon,
            InterpolationOption interpolationOption) {
        super(forecasterId, version, name, description, timeField,
            indices, features, filterQuery,
            windowDelay, shingleSize, uiMetadata, schemaVersion,
            lastUpdateTime, categoryFields, user, resultIndex,
            forecastInterval, interpolationOption);
        if (interval == null) {
            errorMessage =
                    ForecastCommonMessages.NULL_FORECAST_INTERVAL;
            issueType =    ValidationIssueType.FORECAST_INTERVAL;
        } else if (((IntervalTimeConfiguration) interval).getInterval() <= 0) {
            errorMessage =
    ...
    public boolean invalidHorizon(Integer horizonToTest) {
        return horizonToTest != null && (horizonToTest < 1 || horizonToTest > TimeSeriesSettings.MAX_SHINGLE_SIZE * DEFAULT_HORIZON_SHINGLE_RATIO);
    }
     ...
     * @param defaultWindowDelay          default forecaster window delay
     * @return forecaster instance
     * @throws IOException IOException if content can't be parsed correctly
     */
    public static Forecaster parse(
        ...
        TimeConfiguration forecastInterval = defaultForecastInterval == null ? null
                : new IntervalTimeConfiguration(defaultForecastInterval.getMinutes(), ChronoUnit.MINUTES);
        TimeConfiguration windowDelay = defaultForecastWindowDelay == null ? null
        ...
        InterpolationOption interpolationOption = null;
        ...
        while (parser.nextToken() != XContentParser.Token.END_OBJECT) {
            ...
            switch (fieldName) {
                ...
                case FORECAST_INTERVAL_FIELD:
                    try {
                        ...
                        if (e instanceof IllegalArgumentException
                            && e.getMessage().contains(CommonMessages.NEGATIVE_TIME_CONFIGURATION)) {
                            throw new ValidationException(
                ...
                case HORIZON_FIELD:
                    ...
                    break;
                case INTERPOLATION_OPTION_FIELD:
                    interpolationOption = InterpolationOption.parse(parser);
                    break;
                default:
                    parser.skipChildren();
                    break;
    ...
    public XContentBuilder toXContent(XContentBuilder builder, Params params) throws IOException {
        ...
        xContentBuilder
            .field(FORECAST_INTERVAL_FIELD, interval)
    ...
    public boolean equals(Object o) {
        ...
        return super.equals(o)
            && Objects.equal(horizon, forecaster.horizon);
    }
    ...
    public static String validateResultIndex(String resultIndex) {
        if (!resultIndex.startsWith(CUSTOM_RESULT_INDEX_PREFIX)) {
            return CommonMessages.INVALID_RESULT_INDEX_PREFIX;
        }
        return null;
    }
