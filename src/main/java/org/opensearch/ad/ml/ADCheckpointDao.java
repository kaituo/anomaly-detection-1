 ...
 */
public class ADCheckpointDao extends TimeSeriesCheckpointDao<ThresholdedRandomCutForest, ADModelState<EntityModel<ThresholdedRandomCutForest>>> {

    private static final Logger logger = LogManager.getLogger(ADCheckpointDao.class);
    static final String NOT_ABLE_TO_DELETE_CHECKPOINT_MSG = "Cannot delete all checkpoints of detector";
    ...
    public Map<String, Object> toIndexSource(TimeSeriesModelState<?> modelState) throws IOException {
        ...
        if (!(model instanceof EntityModel<?>)) {
            throw new IllegalArgumentException("Excpect model state to be of type EntityModel<ThresholdedRandomCutForest>, but got " + model.getClass());
        }

        @SuppressWarnings("unchecked")
        EntityModel<ThresholdedRandomCutForest> entityModel = ((EntityModel<ThresholdedRandomCutForest>)model);
     ...
     */
    public Optional<String> toCheckpoint(EntityModel<ThresholdedRandomCutForest> model, String modelId) {
        return AccessController.doPrivileged((PrivilegedAction<Optional<String>>) () -> {
            ...
            try {
                ...
                if (model.getModel().isPresent()) {
                    json.addProperty(ENTITY_TRCF, toCheckpoint(model.getModel().get()));
                }
    ...
    public void deleteModelCheckpointByDetectorId(String detectorID) {
        ...
        client.execute(DeleteByQueryAction.INSTANCE, deleteRequest, ActionListener.wrap(response -> {
            ...
            if (exception instanceof IndexNotFoundException) {
                logger.info(TimeSeriesCheckpointDao.INDEX_DELETED_LOG_MSG + " {}", detectorID);
            } else {
     ...
     */
    @Override
    public Optional<ADModelState<EntityModel<ThresholdedRandomCutForest>>> fromEntityModelCheckpoint(Map<String, Object> checkpoint, String modelId, String configId) {
        try {
            return AccessController.doPrivileged((PrivilegedAction<Optional<ADModelState<EntityModel<ThresholdedRandomCutForest>>>>) () -> {
                ...
                EntityModel<ThresholdedRandomCutForest> entityModel = new EntityModel<>(entity, sampleQueue, trcf);

                ADModelState<EntityModel<ThresholdedRandomCutForest>> modelState = new ADModelState<>(
                        entityModel,
                        modelId,
                        configId,
