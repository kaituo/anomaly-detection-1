...
public final class AnomalyDetectorSettings {
    ...
    public static final Setting<String> EVENT_BRIDGE_CELL_ID_HEADER_NAME = new Setting<>(
    ...
    /**
     * Dev/test-only escape hatch. When {@code true}, the REST layer will populate the EventBridge
     * cell-id {@link org.opensearch.common.util.concurrent.ThreadContext} transient from the
     * inbound HTTP header named by {@link #EVENT_BRIDGE_CELL_ID_HEADER_NAME}, if the transient
     * is not already set by an upstream component.
     *
     * <p><b>Must be {@code false} in production.</b> Enabling this lets any client that can reach
     * the REST endpoint directly choose its own cell id by setting the HTTP header, which bypasses
     * the authoritative stamping that an upstream gateway / proxy is expected to perform. Intended
     * solely for local development (e.g. {@code ./gradlew run}) and integration tests where no
     * upstream component is present.
     */
    public static final Setting<Boolean> EVENT_BRIDGE_CELL_ID_FROM_HEADER = Setting
        .boolSetting(
            "plugins.anomaly_detection.event_bridge_cell_id_from_header",
            false,
            Setting.Property.NodeScope,
            Setting.Property.Final
        );

    public static final Setting<String> EVENT_BRIDGE_SCHEDULE_MANAGEMENT_ROLE_NAME = Setting
        .simpleString(
    ...
    public static final Setting<String> EVENT_BRIDGE_SQS_DELIVERY_ROLE_NAME = Setting
        .simpleString("plugins.anomaly_detection.event_bridge.sqs_delivery_role_name", Setting.Property.NodeScope, Setting.Property.Final);

    /**
     * External ID passed on the cross-account {@code sts:AssumeRole} into ScheduleManagerRole.
     * Mirrors {@code plugins.alerting.external_scheduler.external_id}; the ScheduleManagerRole
     * trust policy requires {@code sts:ExternalId} to equal this value. Empty means no external ID
     * is sent; the assume-role code fails closed (rejects) when the target role requires one but
     * this is blank. Guards against confused-deputy exfiltration via request-overridable account
     * routing (see CR-284480892).
     */
    public static final Setting<String> EVENT_BRIDGE_SCHEDULE_MANAGEMENT_EXTERNAL_ID = Setting
        .simpleString(
            "plugins.anomaly_detection.event_bridge.schedule_management_external_id",
            Setting.Property.NodeScope,
            Setting.Property.Final
        );

    /**
     * Allow-list of account IDs the scheduler may route jobs to. A resolved EventBridge cell
     * account that is not in this list is rejected before any cross-account AssumeRole, closing the
     * confused-deputy routing vector where {@code x-scheduler-account-id} is attacker-influenced.
     * An empty list disables the check (bypass) for backward compatibility. Mirrors
     * {@code plugins.alerting.external_scheduler.allowed_account_ids}.
     */
    public static final Setting<List<String>> EVENT_BRIDGE_SCHEDULE_MANAGEMENT_ALLOWED_ACCOUNT_IDS = Setting
        .listSetting(
            "plugins.anomaly_detection.event_bridge.allowed_account_ids",
            List.of(),
            Function.identity(),
            Setting.Property.NodeScope,
            Setting.Property.Final
    ...
    /** Interval for reconciling discovered SQS-owning accounts */
    public static final Setting<TimeValue> SQS_ACCOUNT_RECONCILE_INTERVAL = Setting
        .positiveTimeSetting(
            "plugins.anomaly_detection.sqs.account_reconcile_interval",
    ...
    /** FIFO SQS queue used for AD EventBridge jobs. */
    public static final Setting<String> SQS_QUEUE_NAME = Setting
        .simpleString("plugins.anomaly_detection.sqs.queue_name", "ad-jobs.fifo", Setting.Property.NodeScope, Setting.Property.Final);

    /** Additional AD-owned SQS queues whose CloudWatch metrics contribute to coordinator autoscaling. */
    public static final Setting<List<String>> SQS_EXTRA_QUEUE_NAMES = Setting
        .listSetting(
            "plugins.anomaly_detection.sqs.extra_queue_names",
            List.of(),
            Function.identity(),
            Setting.Property.NodeScope,
            Setting.Property.Final
    ...
    /** This setting overrides the API-path data source endpoint resolver factory class. */
    public static final Setting<String> API_DATA_SOURCE_ENDPOINT_RESOLVER_FACTORY_CLASS = new Setting<>(
