...
public class ADColdStartWorker extends
    ColdStartWorker<ThresholdedRandomCutForest, ADIndex, ADIndexManagement, ADCheckpointDao, ADCheckpointWriteWorker, ADEntityColdStart, ADPriorityCache, AnomalyResult, ThresholdingResult, ADModelManager, ADSaveResultStrategy> {
    ...
    @Override
    protected AnomalyResult createIndexableResult(
        Config config,
        String taskId,
        String modelId,
        ...
        return new AnomalyResult(
            config.getId(),
            taskId,
            ParseUtils.getFeatureData(entry.getValue(), config),
            Instant.ofEpochMilli(entry.getKey() - config.getIntervalInMilliseconds()),
