...
public abstract class HttpNodeCommunicator implements NodeCommunicator {
    ...
    private final String applicationIdHeader;
    private final String dataSourceIdHeader;
    private final String dataPlaneEndpointContextKey;
    ...
    public HttpNodeCommunicator(
        ...
        this.applicationIdHeader = TimeSeriesSettings.APPLICATION_ID_HEADER.get(settings);
        this.dataSourceIdHeader = TimeSeriesSettings.DATA_SOURCE_ID_HEADER.get(settings);
        this.dataPlaneEndpointContextKey = TimeSeriesSettings.DATA_PLANE_ENDPOINT_CONTEXT_KEY.get(settings);
    }
    ...
    private Request buildEntityProfileRequest(EntityProfileRequest request) throws IOException {
        ...
        if (request.getTenantId() != null) {
            ...
            addTenantHeaders(optionsBuilder, request.getTenantId());
    ...
    protected void applyInternalRequestHeaders(Request restRequest, String tenantId) {
        ...
        addTenantHeaders(optionsBuilder, tenantId);
        addCurrentDataPlaneEndpointHeader(optionsBuilder);
        restRequest.setOptions(optionsBuilder);
    }

    private void addCurrentDataPlaneEndpointHeader(RequestOptions.Builder optionsBuilder) {
        if (dataPlaneEndpointContextKey == null || dataPlaneEndpointContextKey.trim().isEmpty()) {
            return;
        }

        DataPlaneClientFactory.RequestContext requestContext = DataPlaneClientFactoryContext.getCurrentRequestContext();
        if (requestContext == null || requestContext.endpoint() == null || requestContext.endpoint().isBlank()) {
            return;
        }

        optionsBuilder.addHeader(dataPlaneEndpointContextKey.trim(), requestContext.endpoint().trim());
    }

    private void addTenantHeaders(RequestOptions.Builder optionsBuilder, String tenantId) {
        if (tenantId == null) {
            return;
        }

        optionsBuilder.addHeader(CommonName.TENANT_ID_HEADER, tenantId);
        int separator = tenantId.indexOf(':');
        if (separator > 0 && separator < tenantId.length() - 1) {
            optionsBuilder.addHeader(applicationIdHeader, tenantId.substring(0, separator));
            optionsBuilder.addHeader(dataSourceIdHeader, tenantId.substring(separator + 1));
        }
    }
    ...
    protected RestClient getNodeRestClient(String endpoint) {
        return RestClientProvider.getModelNodeRestClient(endpoint);
    }
