...
public abstract class HttpNodeCommunicator implements NodeCommunicator {
    ...
    public HttpNodeCommunicator(String restPath, String clusterName, HashRing hashRing) {
    ...
    public void stat(StatsRequest request, ActionListener<StatsNodesResponse> listener) {
        ...
        for (DiscoveryNode node : targetNodes) {
            ...
            RestClient restClient = RestClientProvider.getRestClient(endpoint);
            Request restRequest = buildStatsRequest(request, node.getId());
            restClient.performRequestAsync(restRequest, new ResponseListener() {
                @Override
                public void onSuccess(Response response) {
                    try {
    ...
    protected String buildEntityResultPath(String configId) {
        return new StringBuilder().append(restPath).append("/").append(configId).append("/").append(RestHandlerUtils.ENTITY_RESULT).toString();
    }
    ...
    private Request buildStatsRequest(StatsRequest request, String nodeId) {
        Request restRequest = new Request("GET", buildStatsPath());
        if (nodeId != null) {
            restRequest.addParameter("nodeId", nodeId);
        }
    ...
    private String buildDeleteModelPath(String configId) {
        return new StringBuilder().append(restPath).append("/").append(configId).append("/").append(RestHandlerUtils.DELETE_MODEL).toString();
    }

    private String buildStatsPath() {
        return new StringBuilder().append(restPath).append("/").append(RestHandlerUtils.STATS_NODES).toString();
    }
