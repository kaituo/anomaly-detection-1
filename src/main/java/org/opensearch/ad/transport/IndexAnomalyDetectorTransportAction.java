...
public class IndexAnomalyDetectorTransportAction extends HandledTransportAction<IndexAnomalyDetectorRequest, IndexAnomalyDetectorResponse> {
    ...
    protected void doExecute(Task task, IndexAnomalyDetectorRequest request, ActionListener<IndexAnomalyDetectorResponse> actionListener) {
        ...
        try {
            ...
            if (AnomalyDetectorSettings.AD_MULTI_TENANCY_ENABLED.get(settings) && Strings.isBlank(request.getDataSourceId())) {
                throw new OpenSearchStatusException("Data source ID is required in multi-tenancy mode", RestStatus.BAD_REQUEST);
            }
        } catch (Exception e) {
    ...
    protected void adExecute(
        ...
        try {
            stampTenantMetadata(detector, tenantId, dataSourceId);
        ...
        }, tenantId, dataSourceId, listener);
    }

    private void stampTenantMetadata(AnomalyDetector detector, String tenantId, String dataSourceId) {
        // tenant id and data source id are supplied by headers, not by the detector body.
        detector.setTenantId(tenantId);
        detector.setDataSourceId(dataSourceId);
        if (Strings.isBlank(tenantId)) {
            return;
        }
        TenantComponents tenantComponents = TenantAwareHelper.parseTenantId(tenantId);
        detector.setApplicationId(tenantComponents.applicationId());
        detector.setWorkspaceId(tenantComponents.workspaceId());
    }
    ...
    private void checkIndicesAndExecute(
        ...
        dataAccess.search(searchRequest, TenantContext.user(tenantId, dataSourceId), ActionListener.wrap(r -> {
