                ...
                if (returnTask) {
                    adTaskManager.getAndExecuteOnLatestTasks(detectorID, null, null, ALL_DETECTOR_TASK_TYPES, (taskList) -> {
                        ...
                        if (taskList != null && taskList.size() > 0) {
                            ...
                            if (adTasks.containsKey(ADTaskType.AD_REALTIME_HC_DETECTOR.name())) {
                                realtimeAdTask = Optional.ofNullable(adTasks.get(ADTaskType.AD_REALTIME_HC_DETECTOR.name()));
                            } else if (adTasks.containsKey(ADTaskType.AD_REALTIME_SINGLE_STREAM.name())) {
                                realtimeAdTask = Optional.ofNullable(adTasks.get(ADTaskType.AD_REALTIME_SINGLE_STREAM.name()));
                            }
                            if (adTasks.containsKey(ADTaskType.AD_HISTORICAL_HC_DETECTOR.name())) {
                                historicalAdTask = Optional.ofNullable(adTasks.get(ADTaskType.AD_HISTORICAL_HC_DETECTOR.name()));
                            } else if (adTasks.containsKey(ADTaskType.AD_HISTORICAL_SINGLE_STREAM.name())) {
                                historicalAdTask = Optional.ofNullable(adTasks.get(ADTaskType.AD_HISTORICAL_SINGLE_STREAM.name()));
