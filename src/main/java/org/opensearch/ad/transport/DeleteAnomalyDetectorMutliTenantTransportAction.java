 ...
 */
public class DeleteAnomalyDetectorMutliTenantTransportAction extends BaseDeleteConfigTransportAction<ADTaskCacheManager, ADTaskType, ADTask, ADIndex, ADIndexManagement, ADTaskManager, AnomalyDetector> {
    ...
    public DeleteAnomalyDetectorMutliTenantTransportAction(
        ...
        this.scheduleGroup = AnalysisType.AD.name().toLowerCase(Locale.ROOT);
    }
    ...
    protected void deleteJobDoc(String tenantId, String configId, ActionListener<DeleteResponse> listener) {
        ...
        try {
            schedulerClient
                .deleteSchedule(DeleteScheduleRequest.builder().groupName(scheduleGroup).name(scheduleName).build());
