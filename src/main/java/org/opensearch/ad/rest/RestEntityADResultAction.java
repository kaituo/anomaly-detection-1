...
public class RestEntityADResultAction extends BaseRestHandler {
    ...
    public java.util.List<Route> routes() {
        return ImmutableList.of(
            new Route(
                ...
                String.format(Locale.ROOT, "%s/{%s}/%s", TimeSeriesAnalyticsPlugin.AD_BASE_DETECTORS_URI, DETECTOR_ID, RestHandlerUtils.ENTITY_RESULT)
            )
        );
    }
    ...
    protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
        ...
        EntityResultRequest entityResultRequest = new EntityResultRequest(configId, entities, start, end, AnalysisType.AD, taskId, tenantId);
        return channel -> client.execute(
            EntityADResultAction.INSTANCE,
            entityResultRequest,
