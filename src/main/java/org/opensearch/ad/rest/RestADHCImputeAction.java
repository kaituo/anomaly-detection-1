...
public class RestADHCImputeAction extends BaseRestHandler {
    ...
    public static final String TARGET_NODE_ID_FIELD = "target_node_id";

    private final Settings settings;
    private final ClusterService clusterService;
    private final SDKNodeFilter nodeFilter;

    public RestADHCImputeAction(Settings settings) {
        this(settings, null);
    }

    public RestADHCImputeAction(Settings settings, ClusterService clusterService) {
    ...
    protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
        ...
        while (parser.nextToken() != XContentParser.Token.END_OBJECT) {
            ...
            switch (fieldName) {
                ...
                case DATA_END_MILLIS_FIELD:
                    ...
                    break;
                case TARGET_NODE_ID_FIELD:
                    targetNodeId = parser.textOrNull();
                    break;
                default:
                    parser.skipChildren();
                    break;
        ...
        // This method is called in multi-tenant mode from a broadcasted HTTP request.
        // That node receives the REST call, and RestADHCImputeAction converts it into a transport
        // request targeted at the local node only.
        // This internal endpoint runs on a specific target cluster node chosen by the HTTP communicator.
        DiscoveryNode[] nodes = clusterService != null && clusterService.localNode() != null
            ? new DiscoveryNode[] { clusterService.localNode() }
            : nodeFilter.getEligibleDataNodes();
        if (nodes.length == 0 && clusterService != null && clusterService.localNode() != null) {
            nodes = new DiscoveryNode[] { clusterService.localNode() };
        }

        ADHCImputeRequest imputeRequest = new ADHCImputeRequest(
            configId,
            tenantId,
            taskId,
            dataStartMillis,
        ...
        return channel -> client
            .execute(ADHCImputeAction.INSTANCE, imputeRequest, new RestToXContentListener<ADHCImputeNodesResponse>(channel));
    }
}
