...
public class RestADSingleStreamResultAction extends BaseRestHandler {
    ...
    public RestADSingleStreamResultAction(Settings settings, ADStats adStats) {
    ...
    protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
        ...
        // tenantId is reassigned while parsing the body, so it is not effectively final
        // and cannot be captured by the lambda below. Snapshot it into a final variable.
        ...
        return channel -> {
            incrementExecuteRequestCount(resolvedTenantId);
            client
                .execute(
    ...
    private void incrementExecuteRequestCount(String tenantId) {
        if (adStats != null) {
            adStats.getStat(StatNames.AD_EXECUTE_REQUEST_COUNT.getName()).incrementForTenant(tenantId);
        }
    }
