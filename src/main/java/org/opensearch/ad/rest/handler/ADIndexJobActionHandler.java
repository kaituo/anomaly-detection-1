...
public class ADIndexJobActionHandler extends
    IndexJobActionHandler<ADIndex, ADDelegatingDataManagement, ADTaskCacheManager, ADTaskType, ADTask, ADTaskManager, AnomalyResult, ExecuteADResultResponseRecorder> {

    public ADIndexJobActionHandler(
        Client client,
        ADDelegatingDataManagement indexManagement,
    ...
    protected ResultRequest createResultRequest(String configID, long start, long end, String tenantId) {
        return new AnomalyResultRequest(configID, start, end, tenantId);
    }
    ...
    public void stopConfig(
        ...
        nodeStateManager.getConfig(configId, tenantId, AnalysisType.AD, (config) -> {
            if (!config.isPresent()) {
