...
public class ADEventBridgeTargetResolver {
    ...
    private final Set<String> allowedAccountIds;
    ...
    public ADEventBridgeTargetResolver(Settings settings, JobQueueAccountIdProvider accountProvider) {
        ...
        this.scheduleManagementExternalId = Strings
            .trimToNull(AnomalyDetectorSettings.EVENT_BRIDGE_SCHEDULE_MANAGEMENT_EXTERNAL_ID.get(settings));
        this.allowedAccountIds = Set
            .copyOf(AnomalyDetectorSettings.EVENT_BRIDGE_SCHEDULE_MANAGEMENT_ALLOWED_ACCOUNT_IDS.get(settings));
    ...
    public ADSqsAccountTarget resolveAccount(String accountId) {
        ...
        // Defense against confused-deputy routing: the resolved account can be influenced by a
        // request-supplied cell id (x-scheduler-account-id / config.eventBridgeCellId). Reject any
        // account outside the configured allow-list before we assume a cross-account role into it.
        // An empty allow-list disables the check for backward compatibility. See CR-284480892.
        if (!allowedAccountIds.isEmpty() && !allowedAccountIds.contains(normalizedAccountId)) {
            throw new IllegalArgumentException(
                "Account ID [" + normalizedAccountId + "] is not in the scheduler allow-list " + allowedAccountIds
            );
        }
