...
public class ClusterManagerTaskRegistryTests extends AbstractTimeSeriesTest {
    ...
    private ADCheckpointStore adCheckpointStore;
    private CheckpointDaoInterface<?> forecastCheckpointStore;
    ...
    public void setUp() throws Exception {
        ...
        adCheckpointStore = mock(ADCheckpointStore.class);
        forecastCheckpointStore = mock(CheckpointDaoInterface.class);
    }
    ...
    public void testMultiTenantRegistryContainsExpectedTasksWhenAdMultiTenancyEnabled() {
        ClusterService clusterService = clusterServiceWithPluginSettings(adMultiTenantSettings());
        ClusterManagerTaskRegistry registry = new ClusterManagerTaskRegistry(
            clusterService,
            threadPool,
            client,
    ...
    public void testMultiTenantRegistryContainsExpectedTasksWhenForecastMultiTenancyEnabled() {
        ClusterService clusterService = clusterServiceWithPluginSettings(forecastMultiTenantSettings());
        ClusterManagerTaskRegistry registry = new ClusterManagerTaskRegistry(
            clusterService,
            threadPool,
            client,
    ...
    private Settings adMultiTenantSettings() {
        return Settings
            ...
            .put(ForecastSettings.FORECAST_MULTI_TENANCY_ENABLED.getKey(), false)
            .put(TimeSeriesSettings.SQS_ACCOUNT_PROVIDER_TYPE.getKey(), "plugin_setting")
            .putList(TimeSeriesSettings.SQS_ACCOUNT_IDS.getKey(), "123456789012")
            .put(AnomalyDetectorSettings.EVENT_BRIDGE_SQS_DELIVERY_ROLE_NAME.getKey(), "SchedulerToSQSRole")
            .build();
    }
    ...
    private ClusterService clusterServiceWithPluginSettings(Settings settings) {
        ClusterService clusterService = mock(ClusterService.class);
        ClusterSettings clusterSettings = new ClusterSettings(
            settings,
