...
public class AnomalyDetectorSettingsTests extends OpenSearchTestCase {
    ...
    public void testDataSourceEndpointResolverFactorySettingReturned() {
        ...
        assertTrue(plugin.getSettings().contains(TimeSeriesSettings.DATA_PLANE_ENDPOINT_REGION_SIGNING_ENABLED));
        assertTrue(TimeSeriesSettings.DATA_PLANE_ENDPOINT_REGION_SIGNING_ENABLED.get(Settings.EMPTY));
    }

    public void testTenantRoutingHeaderSettingsReturned() {
        assertTrue(plugin.getSettings().contains(TimeSeriesSettings.APPLICATION_ID_HEADER));
        assertTrue(plugin.getSettings().contains(TimeSeriesSettings.DATA_SOURCE_ID_HEADER));
    }

    public void testTenantRoutingHeaderDefaultsDoNotUseAmznPrefix() {
        assertFalse(TimeSeriesSettings.APPLICATION_ID_HEADER.get(Settings.EMPTY).startsWith("x-amzn-"));
        assertFalse(TimeSeriesSettings.DATA_SOURCE_ID_HEADER.get(Settings.EMPTY).startsWith("x-amzn-"));
    }
