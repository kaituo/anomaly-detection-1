...
public class EntityResultTransportActionTests extends AbstractTimeSeriesTest {
    ...
    public void setUp() throws Exception {
        ...
        coldStarter = mock(ADEntityColdStart.class);

        doAnswer(invocation -> {
            ModelState<ThresholdedRandomCutForest> modelState = invocation.getArgument(0);
            modelState.clear();
            return null;
        }).when(coldStarter).trainModelFromExistingSamples(any(), any(), any(), any());
    ...
    public void testFailToScore() {
        ...
        doThrow(new IllegalArgumentException()).when(spyModelManager).getResult(any(), any(), anyString(), any(), any(), any());
        entityResult = new EntityADResultTransportAction(
            actionFilters,
            transportService,
            spyModelManager,
