...
public class CheckpointReadWorkerTests extends AbstractRateLimitingTest {
    ...
    public void setUp() throws Exception {
        ...
        when(modelManager.getResult(any(), any(), anyString(), any(), any(), anyString())).thenReturn(new ThresholdingResult(0, 1, 0.7));
    ...
    private void regularTestSetUp(RegularSetUpConfig config) {
        ...
        if (config.fullModel) {
            when(modelManager.getResult(any(), any(), anyString(), any(), any(), anyString())).thenReturn(new ThresholdingResult(0, 1, 1));
        } else {
            when(modelManager.getResult(any(), any(), anyString(), any(), any(), anyString())).thenReturn(new ThresholdingResult(0, 0, 0));
        }
    ...
    public void testFailToScore() {
        ...
        state = MLUtil.randomModelState(new RandomModelStateConfig.Builder().fullModel(true).build());
        when(checkpoint.processHCGetResponse(any(), anyString(), anyString())).thenReturn(state);
        doThrow(new IllegalArgumentException()).when(modelManager).getResult(any(), any(), anyString(), any(), any(), anyString());
