...
public class ResultWriteWorkerTests extends AbstractRateLimitingTest {
    ...
    public void testMissingCustomResultIndexCreatesAndRetries() throws IOException {
        ...
        doAnswer(invocation -> {
            ...
            return null;
        }).when(resultHandler).initCustomResultIndexForRetry(eq(customResultIndex), any(), any());
        ...
        verify(resultHandler, times(1)).initCustomResultIndexForRetry(eq(customResultIndex), any(), any());
        verify(resultHandler, times(2)).flush(any(), any(), any());
    }
