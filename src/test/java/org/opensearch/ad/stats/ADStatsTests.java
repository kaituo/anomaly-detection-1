...
public class ADStatsTests extends OpenSearchTestCase {
    ...
    public void setup() {
        ...
        when(cache.getAllModels())
            .thenReturn(new ArrayList<>(Arrays.asList(modelsInformation.get(0), modelsInformation.get(1), entityModel1, entityModel2)));

        IndexUtils indexUtils = mock(IndexUtils.class);

        when(indexUtils.getIndexHealthStatus(any(), anyString())).thenReturn("yellow");
