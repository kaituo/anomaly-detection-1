...
public class ADEventBridgeTargetResolverTests extends OpenSearchTestCase {
    ...
    public void testResolveAccountPropagatesExternalId() {
        JobQueueAccountIdProvider provider = mock(JobQueueAccountIdProvider.class);
        when(provider.getAccountIds()).thenReturn(List.of("793040377150"));
        Settings settings = Settings
            ...
            .put(AnomalyDetectorSettings.EVENT_BRIDGE_SCHEDULE_MANAGEMENT_EXTERNAL_ID.getKey(), "oasisad-scheduler-xacct-b94f7e2a")
        ...
        ADSqsAccountTarget target = resolver.resolveAccount("793040377150");
        assertEquals("oasisad-scheduler-xacct-b94f7e2a", target.getScheduleManagementExternalId());
    }

    public void testResolveAccountHasNullExternalIdWhenUnset() {
        JobQueueAccountIdProvider provider = mock(JobQueueAccountIdProvider.class);
        when(provider.getAccountIds()).thenReturn(List.of("793040377150"));
        ...
        ADSqsAccountTarget target = resolver.resolveAccount("793040377150");
        assertNull(target.getScheduleManagementExternalId());
    }

    public void testResolveAccountAcceptsAllowListedAccount() {
        JobQueueAccountIdProvider provider = mock(JobQueueAccountIdProvider.class);
        when(provider.getAccountIds()).thenReturn(List.of("793040377150", "123456789012"));
        Settings settings = Settings
            .builder()
            .put(TimeSeriesSettings.REGION.getKey(), "us-west-2")
            .put(AnomalyDetectorSettings.EVENT_BRIDGE_SCHEDULE_MANAGEMENT_ROLE_NAME.getKey(), SCHEDULE_MANAGEMENT_ROLE_NAME)
            .putList(
                AnomalyDetectorSettings.EVENT_BRIDGE_SCHEDULE_MANAGEMENT_ALLOWED_ACCOUNT_IDS.getKey(),
                "793040377150",
                "123456789012"
            )
            .put(AnomalyDetectorSettings.EVENT_BRIDGE_SQS_DELIVERY_ROLE_NAME.getKey(), SQS_DELIVERY_ROLE_NAME)
    ...
    public void testResolveAccountRejectsAccountOutsideAllowList() {
        JobQueueAccountIdProvider provider = mock(JobQueueAccountIdProvider.class);
        when(provider.getAccountIds()).thenReturn(List.of("793040377150", "123456789012"));
        Settings settings = Settings
            .builder()
            .put(TimeSeriesSettings.REGION.getKey(), "us-west-2")
            .put(AnomalyDetectorSettings.EVENT_BRIDGE_SCHEDULE_MANAGEMENT_ROLE_NAME.getKey(), SCHEDULE_MANAGEMENT_ROLE_NAME)
            .putList(AnomalyDetectorSettings.EVENT_BRIDGE_SCHEDULE_MANAGEMENT_ALLOWED_ACCOUNT_IDS.getKey(), "793040377150")
            .put(AnomalyDetectorSettings.EVENT_BRIDGE_SQS_DELIVERY_ROLE_NAME.getKey(), SQS_DELIVERY_ROLE_NAME)
            .build();
        ...
        IllegalArgumentException exception = expectThrows(
            ...
            () -> resolver.resolveAccount("123456789012")
        );
        assertTrue(exception.getMessage().contains("allow-list"));
    }

    public void testEmptyAllowListBypassesCheck() {
        JobQueueAccountIdProvider provider = mock(JobQueueAccountIdProvider.class);
        when(provider.getAccountIds()).thenReturn(List.of("793040377150"));

        // settings() sets no allow-list, so the check is disabled and any discovered account resolves.
        ADEventBridgeTargetResolver resolver = new ADEventBridgeTargetResolver(settings(), provider);
