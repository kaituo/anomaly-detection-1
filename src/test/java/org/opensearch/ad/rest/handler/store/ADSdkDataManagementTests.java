...
public class ADSdkDataManagementTests extends AbstractTimeSeriesTest {
    ...
    public void testValidateResultIndexMappingRetriesTransientAossMappingFailure() throws Exception {
        dataManagement = newDataManagement(aossDataManagementSettings());
        AtomicInteger mappingAttempts = new AtomicInteger();
        ...
        doAnswer(invocation -> {
            ...
            int attempt = mappingAttempts.incrementAndGet();
            if (attempt < 3) {
                Response transientFailure = response(403, "{\"error\":{\"type\":\"authorization_exception\"}}");
                listener.onFailure(responseException(transientFailure, request));
            } else {
    ...
    private Settings.Builder aossDataManagementSettings() {
        return dataManagementSettings().put(AnomalyDetectorSettings.REMOTE_METADATA_SERVICE_NAME.getKey(), "aoss");
    }
    ...
    private void cacheRestClient(String endpoint, RestClient client) throws Exception {
        ...
        cache.put(backgroundRestClientCacheKey(endpoint), client);
    }
    ...
    private void invalidateRestClient(String endpoint) throws Exception {
        ...
        cache.invalidate(backgroundRestClientCacheKey(endpoint));
    }

    private String backgroundRestClientCacheKey(String endpoint) {
        return endpoint + "|traffic=" + DataPlaneClientFactory.TrafficClass.BACKGROUND;
    }
