...
public class MultiTenantDualNodeRunRestIT extends AnomalyDetectorRestTestCase {
    ...
    private static final String INTERNAL_API_TOKEN = "integration-test-internal-token";
    ...
    @After
    public void closeModelClient() throws IOException {
        if (modelClient != null) {
            modelClient.close();
            modelClient = null;
        }
    ...
    private Response invokeSingleStreamResult(
        ...
        return TestHelpers
            .makeRequest(
                client,
                "POST",
                TestHelpers.AD_BASE_DETECTORS_URI + "/" + detectorId + "/_single_stream_result",
                Map.<String, String>of(),
                TestHelpers.toHttpEntity(requestBody),
    ...
    private Response invokeEntityResult(
        ...
        return TestHelpers
            .makeRequest(
                client,
                "POST",
                TestHelpers.AD_BASE_DETECTORS_URI + "/" + detectorId + "/_entity_result",
                Map.<String, String>of(),
                TestHelpers.toHttpEntity(requestBody),
                internalTenantHeaders(tenantId)
            );
    ...
    private Response invokeHCImpute(RestClient client, String detectorId, Instant dataStart, Instant dataEnd, String tenantId)
        ...
        return TestHelpers
            .makeRequest(
                client,
                "POST",
                TestHelpers.AD_BASE_DETECTORS_URI + "/" + detectorId + "/_hc_impute",
                Map.<String, String>of(),
                TestHelpers.toHttpEntity(requestBody),
                internalTenantHeaders(tenantId)
            );
    ...
    private Response deleteModel(RestClient client, String detectorId, String tenantId) throws IOException {
        return TestHelpers
            .makeRequest(
                client,
                "POST",
                TestHelpers.AD_BASE_DETECTORS_URI + "/" + detectorId + "/_delete_model",
                Map.<String, String>of(),
                "",
                internalTenantHeaders(tenantId)
            );
    ...
    private long fetchModelCount(RestClient client, String detectorId, String tenantId) throws IOException {
        Response response = TestHelpers
            .makeRequest(
                client,
                "POST",
                TestHelpers.AD_BASE_DETECTORS_URI + "/" + detectorId + "/_node_profile/models",
                Map.<String, String>of(),
                "",
                internalTenantHeaders(tenantId)
            );
