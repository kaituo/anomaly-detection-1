...
public class RestADSingleStreamResultActionTests extends OpenSearchTestCase {
    ...
    public void testPrepareRequestIncrementsExecuteRequestCountWhenStatsProvided() throws Exception {
        ADStats adStats = newAdStats();
        TestRestADSingleStreamResultAction action = new TestRestADSingleStreamResultAction(multiTenantSettings(), adStats);
        ...
        action
            .executePreparedRequest(
                createRequest(
                    "{\"model_id\":\"model-1\",\"start\":1,\"end\":2,\"value_list\":[1.0],\"tenant_id\":\"tenant-a\"}",
                    Map.of(DETECTOR_ID, "detector-1"),
                    tenantHeaders("tenant-a", true)
                ),
                client,
        ...
        verify(client).execute(eq(ADSingleStreamResultAction.INSTANCE), any(), any());
    }
    ...
    private static class TestRestADSingleStreamResultAction extends RestADSingleStreamResultAction {
        TestRestADSingleStreamResultAction(Settings settings, ADStats adStats) {
