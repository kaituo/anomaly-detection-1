...
public class SecureADRestIT extends AnomalyDetectorRestTestCase {
    ...
    public void testStartDetectorWithReadAccess() throws IOException {
        ...
        Exception exception = expectThrows(
            IOException.class,
            () -> { startAnomalyDetector(aliceDetector.getId(), null, bobClient); }
        );
    ...
    public void testGetApiFilterByEnabled() throws IOException {
        ...
        Assert
            .assertTrue(
                exception.getMessage().contains("User does not have permissions to access detector: " + aliceDetector.getId())
            );
    }
    ...
    public void testUpdateApiFilterByEnabledForAdmin() throws IOException {
        ...
        Response response = updateAnomalyDetector(aliceDetector.getConfigId(), newDetector, client());
        Assert.assertEquals(response.getStatusLine().getStatusCode(), 200);
        AnomalyDetector anomalyDetector = getAnomalyDetector(aliceDetector.getConfigId(), aliceClient);
        Assert
            .assertArrayEquals(
    ...
    public void testStopApiFilterByEnabled() throws IOException {
        ...
        Exception exception = expectThrows(
            IOException.class,
            () -> { stopAnomalyDetector(aliceDetector.getId(), catClient, true); }
        );
        Assert
            .assertTrue(
                exception.getMessage().contains("User does not have permissions to access detector: " + aliceDetector.getId())
    ...
    public void testPreviewAnomalyDetectorWithReadAccess() throws IOException {
        ...
        Exception exception = expectThrows(
            IOException.class,
            () -> { previewAnomalyDetector(aliceDetector.getId(), bobClient, input); }
        );
    ...
    public void testPreviewAnomalyDetectorWithNoReadPermissionOfIndex() throws IOException {
        ...
        Exception exception = expectThrows(
            Exception.class,
            () -> { previewAnomalyDetector(aliceDetector.getId(), elkClient, input); }
        );
        Assert
            .assertTrue(
