...
/**
 * Base class for multi-tenant REST integration tests that exercise two OpenSearch test-cluster JVMs.
 *
 * <p>The nodes are not launched by this class. They are started by the Gradle
 * {@code dualNodeMultiTenantIntegTest} task, which attaches two separate single-node test clusters:
 * {@code multiTenantCoordinatorCluster} and {@code multiTenantModelCluster}. The coordinator cluster is
 * exposed to tests through the normal {@link #client()} path via {@code tests.rest.cluster}, while this
 * class resolves the model cluster from {@code tests.model.rest.cluster} in {@link #modelHosts()}.
 *
 * <p>During local runs, the generated OpenSearch home for each node lives under
 * {@code build/testclusters/<clusterName>-0}. The main runtime artifacts are typically:
 * {@code build/testclusters/multiTenantCoordinatorCluster-0/logs},
 * {@code build/testclusters/multiTenantCoordinatorCluster-0/data},
 * {@code build/testclusters/multiTenantModelCluster-0/logs}, and
 * {@code build/testclusters/multiTenantModelCluster-0/data}.
 */
public abstract class AbstractMultiTenantAnomalyDetectorRestTestCase extends AnomalyDetectorRestTestCase {
    ...
    protected static final String DATA_SOURCE_ID = "data-source-1";
    ...
    protected AnomalyDetector createTenantDetector(AnomalyDetector detector, String tenantId) throws IOException {
        ...
        trackTenantDetector(createdDetector.getId(), tenantId);
        return createdDetector;
    }

    protected AnomalyDetector createTenantDetectorWithDataPlaneEndpoint(AnomalyDetector detector, String tenantId) throws IOException {
        AnomalyDetector createdDetector = createAnomalyDetector(detector, true, client(), tenantHeadersWithDataPlaneEndpoint(tenantId));
        trackTenantDetector(createdDetector.getId(), tenantId);
        return createdDetector;
    }

    protected void trackTenantDetector(String detectorId, String tenantId) {
    ...
    protected Response deleteTenantDetector(String detectorId, String tenantId) throws IOException {
        return deleteTenantDetector(detectorId, tenantId, client());
    }

    protected Response deleteTenantDetector(String detectorId, String tenantId, RestClient restClient) throws IOException {
        return TestHelpers
            .makeRequest(
                restClient,
                "DELETE",
    ...
    protected Response startTenantHistoricalDetector(String detectorId, Instant startTime, Instant endTime, String tenantId)
        throws IOException {
        return TestHelpers
            .makeRequest(
                client(),
                ...
                TestHelpers.AD_BASE_DETECTORS_URI + "/" + detectorId + "/_start",
                ImmutableMap.of(),
                TestHelpers.toHttpEntity(new DateRange(startTime, endTime)),
                tenantHeaders(tenantId)
            );
    }

    protected Response startTenantHistoricalDetectorWithDataPlaneEndpoint(
        String detectorId,
        Instant startTime,
        Instant endTime,
        String tenantId
        ...
        return TestHelpers
            .makeRequest(
                ...
                ImmutableMap.of(),
                TestHelpers.toHttpEntity(new DateRange(startTime, endTime)),
                tenantHeadersWithDataPlaneEndpoint(tenantId)
            );
    }

    protected Response stopTenantDetector(String detectorId, String tenantId) throws IOException {
        return stopTenantDetector(detectorId, tenantId, client());
    }

    protected Response stopTenantDetector(String detectorId, String tenantId, RestClient restClient) throws IOException {
        return TestHelpers
            .makeRequest(
                restClient,
                "POST",
    ...
    protected AnomalyDetector withTenantId(AnomalyDetector detector, String tenantId) {
        AnomalyDetector tenantScopedDetector = copyDetector(
            detector,
            detector.getId(),
            detector.getVersion(),
            detector.getName(),
        ...
        tenantScopedDetector.setDataSourceId(DATA_SOURCE_ID);
        return tenantScopedDetector;
    }
    ...
    protected String tenantId(String suffix) {
        return String
            .format(Locale.ROOT, "account-1:application-%s-%s:workspace-1", suffix, randomAlphaOfLength(4).toLowerCase(Locale.ROOT));
    }
    ...
    protected List<Header> tenantHeaders(String tenantId) {
        return List
            .of(
                ...
                new BasicHeader(CommonName.AOSD_DATA_SOURCE_ID_HEADER, DATA_SOURCE_ID),
                new BasicHeader(EVENT_BRIDGE_CELL_ID_HEADER, EVENT_BRIDGE_CELL_ID)
            );
    }

    protected List<Header> tenantHeadersWithDataPlaneEndpoint(String tenantId) {
        List<Header> headers = new ArrayList<>(tenantHeaders(tenantId));
        String endpointHeaderName = dataPlaneEndpointHeaderName();
        headers.removeIf(header -> endpointHeaderName.equalsIgnoreCase(header.getName()));
        headers.add(new BasicHeader(endpointHeaderName, firstModelEndpoint()));
        return headers;
    }

    private String dataPlaneEndpointHeaderName() {
        String headerName = System.getProperty("tests.opensearch.plugins.timeseries.dataplane.endpoint.context_key", "data-source-url");
        return headerName == null || headerName.isBlank() ? "data-source-url" : headerName.trim();
    }

    protected String firstModelEndpoint() {
        HttpHost[] hosts = modelHosts();
        for (HttpHost host : hosts) {
            if (!host.getHostName().contains(":")) {
                return host.toURI();
            }
        }
        return hosts[0].toURI();
    }
    ...
    protected RestClient buildModelClient() throws IOException {
        return buildClient(restClientSettings(), modelHosts());
    }
