...
public class TimeSeriesAnalyticsPluginTests extends OpenSearchTestCase {
    ...
    public void testCreateDataSourceEndpointResolverUsesConfiguredFactoryWhenSettingPresent() {
        ...
        assertEquals("http://datasource-endpoint:9201", resolver.resolve(null, null));
    }
    ...
    public void testDefaultDataSourceEndpointResolverAlwaysReturnsDefaultEndpoint() {
        ...
        assertEquals(DefaultDataSourceEndpointResolver.DEFAULT_ENDPOINT, resolver.resolve(null, null));
        assertEquals(DefaultDataSourceEndpointResolver.DEFAULT_ENDPOINT, resolver.resolve("application-id", "data-source-id"));
    }

    public void testApiDataPlaneFactoryUsesAossAwareDefaultFactoryForAossMode() {
        ...
        assertTrue(factory instanceof ContextAwareDataPlaneClientFactory);
        Object defaultFactory = getFieldValue(factory, "defaultFactory");
        assertTrue(defaultFactory instanceof AossAwareDataPlaneClientFactory);
        assertTrue((Boolean) getFieldValue(defaultFactory, "signAllRequests"));
    }

    public void testApiDataPlaneFactoryUsesAossAwareFactoryForResolvedAossEndpoint() {
        ThreadContext threadContext = new ThreadContext(Settings.EMPTY);
        ThreadPool threadPool = mock(ThreadPool.class);
        when(threadPool.getThreadContext()).thenReturn(threadContext);
        ...
        assertTrue(factory instanceof ContextAwareDataPlaneClientFactory);
        assertTrue(getFieldValue(factory, "defaultFactory") instanceof AossAwareDataPlaneClientFactory);
    }

    public void testApiDataPlaneFactoryUsesAossAwareFactoryForResolvedNonAossEndpoint() {
        ThreadContext threadContext = new ThreadContext(Settings.EMPTY);
        ThreadPool threadPool = mock(ThreadPool.class);
        when(threadPool.getThreadContext()).thenReturn(threadContext);
        ...
        Object defaultFactory = getFieldValue(factory, "defaultFactory");
        assertTrue(defaultFactory instanceof AossAwareDataPlaneClientFactory);
        assertFalse((Boolean) getFieldValue(defaultFactory, "signAllRequests"));
    }
