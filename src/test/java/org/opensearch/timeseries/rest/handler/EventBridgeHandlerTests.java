...
public class EventBridgeHandlerTests extends AbstractTimeSeriesTest {
    ...
    public void testStartJobUsesResolvedMultiAccountTarget() {
        ...
        when(schedulerClient.createSchedule(createCaptor.capture()))
            .thenReturn(CompletableFuture.completedFuture(CreateScheduleResponse.builder().scheduleArn("arn:test").build()));

        EventBridgeHandler handler = new EventBridgeHandler(
            (EventBridgeHandler.SchedulerAsyncClientProvider) target -> schedulerClient,
            "arn:aws:sqs:us-west-2:000000000000:placeholder.fifo",
            "arn:aws:iam::000000000000:role/PlaceholderSchedulerRole",
            "ad",
            AnalysisType.AD,
            Duration.ofHours(1),
            Duration.ofHours(24),
            Clock.systemUTC(),
            new EventBridgeHandler.MultiAccountTargetResolver() {
                @Override
                public EventBridgeHandler.EventBridgeTarget resolveConfigTarget(org.opensearch.timeseries.model.Config config) {
                    return new EventBridgeHandler.EventBridgeTarget(
                        "123456789012",
                        "arn:aws:sqs:us-west-2:123456789012:ad-jobs.fifo",
                        "arn:aws:iam::123456789012:role/ADScheduleManagementRole",
                        "arn:aws:iam::123456789012:role/EventBridgeSchedulerSqsDeliveryRole"
                    );
                }

                @Override
                public EventBridgeHandler.EventBridgeTarget resolveMaintenanceTarget() {
        ...
        org.opensearch.timeseries.model.Config config = mock(org.opensearch.timeseries.model.Config.class);
        when(config.getId()).thenReturn("detector-1");
        when(config.getTenantId()).thenReturn("tenant-a");
        when(config.getInferredFrequencyInMinutes()).thenReturn(5L);
        when(config.getInferredFrequency()).thenReturn(new IntervalTimeConfiguration(5, ChronoUnit.MINUTES));

        CompletableFuture<JobResponse> future = new CompletableFuture<>();
        handler.startJob(config, null, Clock.systemUTC(), ActionListener.wrap(future::complete, future::completeExceptionally));

        assertEquals("detector-1", future.join().getId());
        assertEquals("arn:aws:sqs:us-west-2:123456789012:ad-jobs.fifo", createCaptor.getValue().target().arn());
        assertEquals("arn:aws:iam::123456789012:role/EventBridgeSchedulerSqsDeliveryRole", createCaptor.getValue().target().roleArn());
    }

    public void testStartJobUsesScheduleManagementTargetForSchedulerClient() {
        SchedulerAsyncClient schedulerClient = mock(SchedulerAsyncClient.class);
        when(schedulerClient.createSchedule(any(CreateScheduleRequest.class)))
            .thenReturn(CompletableFuture.completedFuture(CreateScheduleResponse.builder().scheduleArn("arn:test").build()));

        AtomicReference<EventBridgeHandler.EventBridgeTarget> selectedTarget = new AtomicReference<>();
        EventBridgeHandler.SchedulerAsyncClientProvider schedulerClientProvider = new EventBridgeHandler.SchedulerAsyncClientProvider() {
            @Override
            public SchedulerAsyncClient getClient(EventBridgeHandler.EventBridgeTarget target) {
        ...
        assertEquals("detector-1", future.join().getId());
        assertEquals("123456789012", selectedTarget.get().getAccountId());
        assertEquals("arn:aws:iam::123456789012:role/ADScheduleManagementRole", selectedTarget.get().getScheduleManagementRoleArn());
    }

    public void testStartJobUpdateUsesResolvedTargetRole() {
        SchedulerAsyncClient schedulerClient = mock(SchedulerAsyncClient.class);
        when(schedulerClient.createSchedule(any(CreateScheduleRequest.class)))
            .thenReturn(CompletableFuture.failedFuture(ConflictException.builder().message("exists").build()));
        ...
        assertEquals("detector-1", future.join().getId());
        assertEquals("arn:aws:iam::123456789012:role/EventBridgeSchedulerSqsDeliveryRole", updateCaptor.getValue().target().roleArn());
    }

    public void testStopJobUsesConfigEventBridgeAccount() {
        SchedulerAsyncClient schedulerClient = mock(SchedulerAsyncClient.class);
        when(schedulerClient.getSchedule(any(GetScheduleRequest.class))).thenReturn(CompletableFuture.completedFuture(existingSchedule()));
        when(schedulerClient.updateSchedule(any(UpdateScheduleRequest.class)))
            .thenReturn(CompletableFuture.completedFuture(UpdateScheduleResponse.builder().scheduleArn("arn:test").build()));

        AtomicReference<EventBridgeHandler.EventBridgeTarget> selectedTarget = new AtomicReference<>();
        EventBridgeHandler handler = new EventBridgeHandler(target -> {
            selectedTarget.set(target);
            return schedulerClient;
        },
            "arn:aws:sqs:us-west-2:000000000000:placeholder.fifo",
            ...
            Duration.ofHours(24),
            Clock.systemUTC(),
            accountAwareResolver()
        ) {
        };

        CompletableFuture<JobResponse> future = new CompletableFuture<>();
        handler
            .stopJob("detector-1", "tenant-a", "123456789012", null, ActionListener.wrap(future::complete, future::completeExceptionally));
    ...
    public void testDeleteScheduleUsesConfigAccountWhenPresent() {
        SchedulerAsyncClient schedulerClient = mock(SchedulerAsyncClient.class);
        when(schedulerClient.deleteSchedule(any(DeleteScheduleRequest.class)))
            .thenReturn(CompletableFuture.completedFuture(DeleteScheduleResponse.builder().build()));
        ...
        handler.deleteSchedule("tenant-a", "detector-1", "123456789012");

        assertEquals("123456789012", selectedTarget.get().getAccountId());
        verify(schedulerClient).deleteSchedule(any(DeleteScheduleRequest.class));
    }

    public void testDeleteScheduleFansOutWhenConfigAccountMissing() {
        SchedulerAsyncClient schedulerClient = mock(SchedulerAsyncClient.class);
        when(schedulerClient.deleteSchedule(any(DeleteScheduleRequest.class)))
            .thenReturn(CompletableFuture.completedFuture(DeleteScheduleResponse.builder().build()));

        java.util.List<String> selectedAccounts = new java.util.ArrayList<>();
        EventBridgeHandler handler = new EventBridgeHandler(target -> {
            selectedAccounts.add(target.getAccountId());
            return schedulerClient;
        },
            "arn:aws:sqs:us-west-2:000000000000:placeholder.fifo",
            "arn:aws:iam::000000000000:role/PlaceholderSchedulerRole",
        ...
        handler.deleteSchedule("tenant-a", "detector-1", null);

        assertEquals(java.util.List.of("123456789012", "210987654321"), selectedAccounts);
        verify(schedulerClient, org.mockito.Mockito.times(2)).deleteSchedule(any(DeleteScheduleRequest.class));
    }

    public void testStartHourlyCronUsesResolvedMaintenanceTarget() {
        SchedulerAsyncClient schedulerClient = mock(SchedulerAsyncClient.class);
        when(schedulerClient.getSchedule(any(GetScheduleRequest.class)))
            .thenReturn(CompletableFuture.failedFuture(ResourceNotFoundException.builder().message("missing").build()));
        ...
        EventBridgeHandler handler = new EventBridgeHandler(
            ...
            new EventBridgeHandler.MultiAccountTargetResolver() {
                ...
                public EventBridgeHandler.EventBridgeTarget resolveMaintenanceTarget() {
                    return new EventBridgeHandler.EventBridgeTarget(
                        "210987654321",
                        "arn:aws:sqs:us-west-2:210987654321:ad-jobs.fifo",
                        "arn:aws:iam::210987654321:role/ADScheduleManagementRole",
                        "arn:aws:iam::210987654321:role/EventBridgeSchedulerSqsDeliveryRole"
                    );
                }
            }
        ) {
        ...
        CompletableFuture<JobResponse> future = new CompletableFuture<>();
        handler.startHourlyCron(ActionListener.wrap(future::complete, future::completeExceptionally));

        assertEquals("HourlyCron", future.join().getId());
        assertEquals("arn:aws:sqs:us-west-2:210987654321:ad-jobs.fifo", createCaptor.getValue().target().arn());
        assertEquals("arn:aws:iam::210987654321:role/EventBridgeSchedulerSqsDeliveryRole", createCaptor.getValue().target().roleArn());
    }
    ...
    public void testStartDailyS3CheckpointCleanupUpdatesEnabledSchedule() {
        ...
        when(schedulerClient.getSchedule(any(GetScheduleRequest.class)))
            .thenReturn(
                CompletableFuture
                    .completedFuture(GetScheduleResponse.builder().name("DailyS3CheckpointCleanup").state(ScheduleState.ENABLED).build())
            );
    ...
    private GetScheduleResponse existingSchedule() {
        return GetScheduleResponse
            .builder()
            .name(EventBridgeHandler.buildScheduleName(AnalysisType.AD, "tenant-a", "detector-1"))
            .state(ScheduleState.ENABLED)
            .scheduleExpression("rate(5 minutes)")
            .flexibleTimeWindow(FlexibleTimeWindow.builder().mode(FlexibleTimeWindowMode.FLEXIBLE).maximumWindowInMinutes(1).build())
            .target(
                Target
                    .builder()
                    .arn("arn:aws:sqs:us-west-2:123456789012:ad-jobs.fifo")
                    .roleArn("arn:aws:iam::123456789012:role/EventBridgeSchedulerSqsDeliveryRole")
                    .build()
            )
            .build();
    }

    private EventBridgeHandler.MultiAccountTargetResolver accountAwareResolver() {
        return new EventBridgeHandler.MultiAccountTargetResolver() {
            @Override
            public EventBridgeHandler.EventBridgeTarget resolveConfigTarget(org.opensearch.timeseries.model.Config config) {
                throw new UnsupportedOperationException("Not used in this test");
            }
            ...
            @Override
            public EventBridgeHandler.EventBridgeTarget resolveAccountTarget(String accountId) {
                return target(accountId);
            }

            @Override
            public java.util.List<EventBridgeHandler.EventBridgeTarget> resolveAllTargets() {
                return java.util.List.of(target("123456789012"), target("210987654321"));
            }
        };
    }

    private EventBridgeHandler.EventBridgeTarget target(String accountId) {
        return new EventBridgeHandler.EventBridgeTarget(
            accountId,
            "arn:aws:sqs:us-west-2:" + accountId + ":ad-jobs.fifo",
            "arn:aws:iam::" + accountId + ":role/ADScheduleManagementRole",
            "arn:aws:iam::" + accountId + ":role/EventBridgeSchedulerSqsDeliveryRole"
        );
    }
