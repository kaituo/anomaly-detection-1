...
public class NodeStateManagerTests extends AbstractTimeSeriesTest {
    ...
    public void testConfigStateClearMarkerDropsOnlyOlderRequestsAndExpires() {
        ...
        stateManager.markConfigStateCleared("tenant-1", "config-1");

        assertTrue(stateManager.isConfigStateClearedAfter("tenant-1", "config-1", 999L));
        assertTrue(stateManager.isConfigStateClearedAfter("tenant-1", "config-1", 1_000L));
        assertFalse(stateManager.isConfigStateClearedAfter("tenant-1", "config-1", 1_001L));
        assertFalse(stateManager.isConfigStateClearedAfter("tenant-2", "config-1", 999L));

        when(clock.instant()).thenReturn(clearedAt.plus(duration).plusMillis(1));
        stateManager.maintenance();

        assertFalse(stateManager.isConfigStateClearedAfter("tenant-1", "config-1", 999L));
    }
    ...
    public void testGetAnomalyDetectorUsesInjectedConfigStore() throws Exception {
        ConfigDocumentStore configDocumentStore = mock(ConfigDocumentStore.class);
        NodeStateManager configStoreBackedStateManager = new NodeStateManager(
            client,
            xContentRegistry(),
            settings,
            clientUtil,
        ...
        configStoreBackedStateManager.getConfig(detector.getId(), "tenant-1", AnalysisType.AD, true, future);
        ...
        assertEquals(detector.getId(), requestCaptor.getValue().id());
        assertEquals("tenant-1", tenantCaptor.getValue().getTenantId());
    }
