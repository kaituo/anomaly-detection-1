...
public class SdkStateManagerTests extends AbstractTimeSeriesTest {
    ...
    public void testConfigStateClearMarkerDropsOnlyOlderRequestsAndExpires() {
        Instant clearedAt = Instant.ofEpochMilli(1_000L);
        when(clock.instant()).thenReturn(clearedAt);

        stateManager.markConfigStateCleared(TENANT_ID, CONFIG_ID);

        assertTrue(stateManager.isConfigStateClearedAfter(TENANT_ID, CONFIG_ID, 999L));
        assertTrue(stateManager.isConfigStateClearedAfter(TENANT_ID, CONFIG_ID, 1_000L));
        assertFalse(stateManager.isConfigStateClearedAfter(TENANT_ID, CONFIG_ID, 1_001L));
        assertFalse(stateManager.isConfigStateClearedAfter("other-tenant", CONFIG_ID, 999L));

        when(clock.instant()).thenReturn(clearedAt.plus(Duration.ofHours(1)).plusMillis(1));
        stateManager.maintenance();

        assertFalse(stateManager.isConfigStateClearedAfter(TENANT_ID, CONFIG_ID, 999L));
    }
    ...
    public void testGetJobLoadsAndCachesJobFromSchedule() {
        ...
        when(eventBridgeHandler.getJobFromSchedule(TENANT_ID, CONFIG_ID, null)).thenReturn(Optional.of(job));
        ...
        verify(eventBridgeHandler, times(1)).getJobFromSchedule(TENANT_ID, CONFIG_ID, null);
    }

    public void testGetJobReturnsEmptyWhenScheduleMissing() {
        EventBridgeHandler eventBridgeHandler = mock(EventBridgeHandler.class);
        SdkStateManager manager = createStateManager(eventBridgeHandler);
        when(eventBridgeHandler.getJobFromSchedule(TENANT_ID, CONFIG_ID, null)).thenReturn(Optional.empty());
    ...
    public void testGetJobLoadsMaintenanceJobFromMaintenanceSchedule() {
        ...
        verify(eventBridgeHandler, times(0)).getJobFromSchedule(any(), any(), any());
    }
    ...
    public void testGetJobPropagatesHandlerException() throws InterruptedException {
        ...
        SdkStateManager manager = createStateManager(eventBridgeHandler);
        RuntimeException failure = new RuntimeException("boom");
        when(eventBridgeHandler.getJobFromSchedule(TENANT_ID, CONFIG_ID, null)).thenThrow(failure);
