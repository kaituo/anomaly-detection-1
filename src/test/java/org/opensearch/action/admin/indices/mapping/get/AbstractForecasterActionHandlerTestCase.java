...
public class AbstractForecasterActionHandlerTestCase extends AbstractTimeSeriesTest {
    ...
    public void setUp() throws Exception {
        ...
        doAnswer(invocation -> {
            String indexOrAliasName = invocation.getArgument(0);
            @SuppressWarnings("unchecked")
            ActionListener<Boolean> listener = invocation.getArgument(1);
            String tenantId = invocation.getArgument(2);
            try {
                listener
                    .onResponse(
                        forecastIndexManagement.doesResultIndexExists(indexOrAliasName, tenantId)
                            || forecastIndexManagement.doesResultAliasExists(indexOrAliasName, tenantId)
                    );
            } catch (Exception e) {
                listener.onFailure(e);
            }
            return null;
        }).when(forecastIndexManagement).doesResultIndexOrAliasExists(any(), any(), nullable(String.class), any());
